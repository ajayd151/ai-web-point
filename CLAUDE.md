...
- `lib/vo-run.js`: `sourceCandidates`, `processCandidate` (hard filters, analysis, store rule feed-or-cart, enrichment, DM match, score, product pick, messages), `stepRun` (Apify phases, stop rules).
- `lib/vo-jobs.js`: runs, `startScheduled`, `recheck` (cadence and forced recount), `refreshProducts`, `sendEmail`, follow-ups, `linkedinTick` (acceptance, Pending 7 d, withdraw 21 d, replies with sentiment, one request at a random 10 to 30 min gap inside the send window and caps, auto-pause), `linkedinSend` (human in the loop), alerts (`newLeadAlerts`, `replyAlerts`, `testAlerts`), `dailyReport`, `tick`.
