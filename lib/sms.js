...
// List recent OUTBOUND messages we sent To a given number (Twilio Messages API). Used to backfill
// delivery status for auto-sends whose SID we never stored. Returns [{sid, status, dateSent, body}].
async function listMessagesTo(to, pageSize) {
  const sid = process.env.TWILIO_ACCOUNT_SID;
  if (!sid || !process.env.TWILIO_AUTH_TOKEN || !to) return [];
  const auth = 'Basic ' + Buffer.from(sid + ':' + process.env.TWILIO_AUTH_TOKEN).toString('base64');
  const u = new URL('https://api.twilio.com/2010-04-01/Accounts/' + sid + '/Messages.json');
  u.searchParams.set('To', to);
  u.searchParams.set('PageSize', String(Math.min(Math.max(Number(pageSize) || 50, 1), 100)));
  try {
    const r = await fetch(u.toString(), { headers: { Authorization: auth } });
    const d = await r.json().catch(() => ({}));
    return Array.isArray(d.messages)
      ? d.messages.map((m) => ({ sid: m.sid, status: String(m.status || '').toLowerCase(), dateSent: m.date_sent || m.date_created, body: m.body || '', direction: m.direction || '' }))
      : [];
  } catch (e) { return []; }
}

module.exports = { smsConfigured, ukMobile, sendSms, lookupPhone, listMessagesTo, optOutToken, verifyOptOutToken, optOutUrl };
