...
async function wooProducts(domain, opts) {
  const o = opts || {};
  const ctrl = new AbortController(); const t = setTimeout(() => ctrl.abort(), o.timeoutMs || 6000);
  try {
    const r = await fetch('https://' + domain + '/wp-json/wc/store/v1/products?per_page=50', { signal: ctrl.signal, headers: { 'User-Agent': BROWSER_UA, Accept: 'application/json' }, redirect: 'follow' });
    clearTimeout(t);
