...
function normaliseAdRow(r) {
  ...
  return {
    ...
    copy: cut(String((s.body && s.body.text) || r.adText || r.ad_text || r.body || r.text || r.caption || ''), 400),
...
// cut on whole characters so an emoji is never split into a lone surrogate
function cut(str, n) { return Array.from(String(str || '')).slice(0, n).join(''); }
...
async function apifyStartRun(opts) {
  ...
  const r = await fetchRetry('https://api.apify.com/v2/acts/' + apifyActor() + '/runs?token=' + encodeURIComponent(KEY.apify()) + '&timeout=900', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ urls: urls, count: limit, limitPerSource: Math.ceil(limit / urls.length), scrapeAdDetails: false, scrapePageAds: { activeStatus: 'active', countryCode: o.country || 'US', sortBy: 'date' } }),
  }, { retries: 1, timeoutMs: 20000 });
  const d = await r.json().catch(() => ({}));
  if (!r.ok || !d.data || !d.data.id) throw new Error('Apify could not start the actor: HTTP ' + r.status + ' ' + String((d.error && (d.error.message || d.error.type)) || JSON.stringify(d).slice(0, 200)).slice(0, 300));
  return { run_id: d.data.id, dataset_id: d.data.defaultDatasetId, status: d.data.status, started_at: new Date().toISOString() };
}
// The actor scrapes a page's ads when given the page URL itself (https://www.facebook.com/<page id or handle>); the scrapePageAds settings apply to those.
function pageAdsUrl(pageId) { return 'https://www.facebook.com/' + encodeURIComponent(String(pageId)); }
...
async function apifyStartPageCount(pageIds, country, perPage) {
  ...
  const r = await fetchRetry('https://api.apify.com/v2/acts/' + apifyActor() + '/runs?token=' + encodeURIComponent(KEY.apify()) + '&timeout=900', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ urls: ids.map((id) => ({ url: pageAdsUrl(id) })), count: ids.length * per, limitPerSource: per, scrapeAdDetails: false, scrapePageAds: { activeStatus: 'active', sortBy: 'date', countryCode: (country && country !== 'ALL') ? String(country).toUpperCase() : 'ALL' } }),
  }, { retries: 1, timeoutMs: 20000 });
  const d = await r.json().catch(() => ({}));
  if (!r.ok || !d.data || !d.data.id) throw new Error('Apify could not start the page count: HTTP ' + r.status + ' ' + String((d.error && (d.error.message || d.error.type)) || JSON.stringify(d).slice(0, 200)).slice(0, 300));
...
// One brand page, newest N active ads, synchronously (small enough for one function call). Dry run: fixture counts.
async function apifyPageAds(pageId, country, per) {
  if (dryRun('apify')) { const rows = fixtureAdRows([], country === 'ALL' ? '' : country, false, 5000).filter((r) => r.pageId === 'fx-' + pageId || String(r.pageId) === String(pageId)); return { rows: rows.map(normaliseAdRow), cost: 0, dry: true }; }
  const n = Math.max(10, Math.min(60, Number(per) || 30));
  const r = await fetchRetry('https://api.apify.com/v2/acts/' + apifyActor() + '/run-sync-get-dataset-items?token=' + encodeURIComponent(KEY.apify()) + '&timeout=50&format=json&clean=true', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ urls: [{ url: pageAdsUrl(pageId) }], count: n, limitPerSource: n, scrapeAdDetails: false, scrapePageAds: { activeStatus: 'active', sortBy: 'date', countryCode: (country && country !== 'ALL') ? String(country).toUpperCase() : 'ALL' } }),
  }, { retries: 0, timeoutMs: 58000 });
  if (!r.ok) { let body = ''; try { body = (await r.text()).slice(0, 200); } catch (e) {} throw new Error('Apify page ads HTTP ' + r.status + ' ' + body); }
  const data = await r.json().catch(() => []); const raw = Array.isArray(data) ? data : (data.items || []);
...
function groupAds(rows, opts) {
  ...
  for (const g of byPage.values()) {
    ...
    out.push({ page_id: g.page_id, page_name: g.page_name, brand: g.page_name, domain: domain, website: domain,
      ...
      ad_samples: g.rows.slice(0, 10).map((r) => ({ ad_id: r.ad_id, start_date: r.start_date, is_video: r.is_video, duration: r.duration, page_name: r.page_name, copy: cut(r.copy, 200), thumbnail: r.thumbnail, link_url: r.link_url })) });
  }
  return out;
}
...
async function apolloPeople(domain, titles) {
  ...
  const r = await fetchRetry('https://api.apollo.io/api/v1/mixed_people/api_search', { method: 'POST', headers: apolloHeaders(), body: JSON.stringify({ q_organization_domains_list: [domain], person_titles: titles, include_similar_titles: true, page: 1, per_page: 10 }) }, { retries: 2, timeoutMs: 15000 });
...
// WooCommerce Store API (public, no key): /wp-json/wc/store/v1/products. Same product shape as the Shopify reader.
function mapWooProducts(list, domain) {
  return (Array.isArray(list) ? list : []).filter((p) => p && p.slug && p.name).map((p) => ({
    handle: String(p.slug), title: String(p.name).replace(/&amp;/g, '&').replace(/\u00a0/g, ' ').replace(/\s+/g, ' ').trim(),
    price: p.prices && p.prices.price != null ? Number(p.prices.price) / Math.pow(10, Number(p.prices.currency_minor_unit) || 2) : null,
    tags: [].concat(p.categories || [], p.tags || []).map((t) => String(t && t.name ? t.name : t)).filter(Boolean),
    created_at: null, reviews_count: Number(p.review_count) || 0,
    images: (p.images || []).map((i) => ({ src: String(i.src || ''), alt: String(i.alt || i.name || '') })).filter((i) => i.src),
    url: String(p.permalink || ('https://' + domain + '/product/' + p.slug + '/')).replace(/\\\//g, '/'),
  }));
}
async function wooProducts(domain, opts) {
  const o = opts || {};
  const ctrl = new AbortController(); const t = setTimeout(() => ctrl.abort(), o.timeoutMs || 15000);
  try {
    const r = await fetch('https://' + domain + '/wp-json/wc/store/v1/products?per_page=30&_fields=id,name,slug,permalink,prices,images,categories,tags,review_count', { signal: ctrl.signal, headers: { 'User-Agent': BROWSER_UA, Accept: 'application/json' }, redirect: 'follow' });
    clearTimeout(t);
    if (!r.ok) return { is_woo: false, products: [], status: r.status };
    const d = await r.json().catch(() => null);
    const products = mapWooProducts(d, domain);
    return products.length ? { is_woo: true, products: products, skus: products.length } : { is_woo: false, products: [] };
  } catch (e) { clearTimeout(t); return { is_woo: false, products: [], error: e.message }; }
}
// The store reader chain: Shopify feed, then WooCommerce, then the ads' landing links. source: shopify | woo | ads | unknown
async function storeProducts(domain, adSamples, opts) {
  const shop = await shopifyProducts(domain, opts);
  if (shop.is_shopify) return { is_store: true, source: 'shopify', products: shop.products, skus: shop.skus, shop: shop };
  if (!(opts && opts.dry)) {
    const woo = await wooProducts(domain, opts);
    if (woo.is_woo) return { is_store: true, source: 'woo', products: woo.products, skus: woo.skus, shop: shop };
    shop.woo_status = woo.status || (woo.error ? 'error ' + woo.error : 'no products');
  }
  const fromAds = productsFromAds(adSamples || [], domain);
  if (fromAds.length) return { is_store: false, source: 'ads', products: fromAds, skus: shop.skus, shop: shop };
  return { is_store: false, source: 'unknown', products: [], skus: shop.skus, shop: shop };
}
async function shopifyProducts(domain, opts) {
  ...
  try {
    const r = await fetch('https://' + domain + '/products.json?limit=50', { signal: ctrl.signal, headers: { 'User-Agent': BROWSER_UA, Accept: 'application/json' }, redirect: 'follow' });
    clearTimeout(t);
...
async function hasStore(domain, shop) {
  ...
  try { const r = await fetch('https://' + domain + '/cart', { signal: ctrl.signal, redirect: 'follow', headers: { 'User-Agent': BROWSER_UA } }); clearTimeout(t); return r.status < 400; } catch (e) { clearTimeout(t); return false; }
}
...
// When a store blocks readers (no products.json, bot wall on the homepage), the ads themselves still say what the brand
// sells: each ad's landing link usually points at a product page. Build a product list from those slugs.
function productsFromAds(samples, domain) {
  const out = new Map();
  for (const a of samples || []) {
    const u = String(a.link_url || ''); if (!u) continue;
    let path = ''; try { const x = new URL(u.startsWith('http') ? u : 'https://' + u); if (domain && !x.hostname.replace(/^www\./, '').endsWith(String(domain).replace(/^www\./, ''))) continue; path = x.pathname; } catch (e) { continue; }
    const m = path.match(/\/(?:products?|shop|item|p)\/([a-z0-9][a-z0-9-]{2,})\/?$/i) || path.match(/\/collections\/[^/]+\/products\/([a-z0-9][a-z0-9-]{2,})/i);
    if (!m) continue;
    const handle = m[1].toLowerCase(); if (out.has(handle)) continue;
    const title = handle.replace(/-/g, ' ').replace(/\b([a-z])/g, (c) => c.toUpperCase()).replace(/\b(\d+)(ml|oz|g|mg|ct|pk)\b/gi, '$1$2');
    out.set(handle, { handle: handle, title: title, price: null, tags: [], created_at: null, images: [], url: 'https://' + String(domain || '').replace(/^www\./, '') + path.replace(/\/$/, ''), from_ads: true, ad_count: 0 });
  }
  for (const a of samples || []) { const u = String(a.link_url || ''); for (const p of out.values()) if (u.includes('/' + p.handle)) p.ad_count++; }
  return Array.from(out.values()).sort((a, b) => b.ad_count - a.ad_count);
}
function pickProduct(products, heroName, featuredHandles, opts) {
  ...
  const real = list.filter((p) => !merch(p)); const pool = real.length ? real : list; // a tee is never the sample video subject while a real product exists
  // rank: the hero named in the ads, then anything matching the campaign keywords, then homepage featured, then new, reviews, clean URL
  const ranked = pool.slice().sort((a, b) => (isHero(b) - isHero(a)) || (heroPartial(b) - heroPartial(a)) || (hintHits(b) - hintHits(a)) || (featIdx(a) - featIdx(b)) || (isNew(b) - isNew(a)) || ((b.reviews_count || 0) - (a.reviews_count || 0)) || (cleanScore(a.handle) - cleanScore(b.handle)));
  const reasonFor = (p) => isHero(p) ? 'named most in their current ads' : (heroPartial(p) ? 'closest to the product named in their ads' : (hintHits(p) ? 'matches what the campaign is about' : (featIdx(p) < 99 ? 'featured on their homepage' : (isNew(p) ? 'tagged as new or seasonal' : 'best remaining product'))));
  // the ranked shortlist: up to three products that pass the photo rule, with the reason each earned its place
  const candidates = [];
  if (o.source === 'woo') { // the Store API lists a photo or two per product: rank as usual, show the count, do not fail on it
    for (const p of ranked.slice(0, 3)) candidates.push({ url: p.url, name: p.title, photos: realPhotoCount(p), why: reasonFor(p) + ', ' + realPhotoCount(p) + ' photo' + (realPhotoCount(p) === 1 ? '' : 's') + ' listed' });
    const top = ranked[0]; const n = realPhotoCount(top);
    return { url: top.url, name: top.title, check: n >= 3 ? 'Pass (' + n + ')' : 'Unverified', why: reasonFor(top) + ', from their WooCommerce store, ' + n + ' photo' + (n === 1 ? '' : 's') + ' listed', candidates_tried: 1, candidates: candidates };
  }
  if (o.source === 'ads') { // no photos to check: the store blocks readers, these came from the ads' landing links
    for (const p of ranked.slice(0, 3)) candidates.push({ url: p.url, name: p.title, photos: 0, why: (p.ad_count > 1 ? 'linked from ' + p.ad_count + ' of their ads' : 'linked from their ads') + ', photos unchecked' });
    const top = ranked[0];
    return { url: top.url, name: top.title, check: 'Unverified', why: 'their store blocks readers, taken from the ads, photos unchecked', candidates_tried: 1, candidates: candidates };
  }
  for (const p of ranked) { const n = realPhotoCount(p); if (n >= 3) candidates.push({ url: p.url, name: p.title, photos: n, why: reasonFor(p) }); if (candidates.length >= 3) break; }
  let weak = null; let tried = 0;
  for (const p of ranked) {
    tried++;
    const n = realPhotoCount(p);
    const reason = reasonFor(p);
    if (n >= 3) return { url: p.url, name: p.title, check: 'Pass (' + n + ')', why: reason + ', ' + n + ' real photos', candidates_tried: tried, candidates: candidates };
    if (n === 2 && !weak) weak = { url: p.url, name: p.title, check: 'Weak pass (2)', why: reason + ', only 2 clean photos', candidates_tried: tried, candidates: candidates };
    if (tried >= (opts && opts.maxTries || 8)) break;
  }
  if (weak) return weak;
  const top = ranked[0]; const n = realPhotoCount(top);
  return { url: top.url, name: top.title, check: 'FAIL (' + n + ')', why: 'no product has 3 real photos, best was ' + n, candidates_tried: tried, candidates: candidates };
}
...
async function suggestKeywords(industry, countries, language, opts) {
  const ind = String(industry || '').trim(); if (!ind) return { keywords: [], translated: {} };
  const o = opts || {};
  if (o.mode === 'neighbours') {
    // the same industry keeps returning the same advertisers, so widen to products the same kind of DTC brand sells
    const avoid = (o.avoid || []).slice(0, 60).join(', ');
    const p2 = await aiJson('A DTC ecommerce brand sells "' + ind + '" and advertises on Meta. Countries: ' + (countries || ['US']).join(', ') + '.\nList 12 lower-case shopper search terms (1 to 4 words each) for NEIGHBOURING product categories that similar DTC brands sell and advertise, for example other supplement formats, adjacent wellness or fitness products. No brand names, no research phrases (benefits, review, dosage, how to). Do not repeat any of these: ' + avoid + '.\nReturn {"keywords": [...]}', 'kwn:' + ind + ':' + (o.round || 0));
    if (p2 && Array.isArray(p2.keywords) && p2.keywords.length) return { keywords: p2.keywords.map((k) => String(k).toLowerCase().trim()).filter(Boolean).slice(0, 12), translated: {} };
    const base = ind.toLowerCase().replace(/gummies|gummy/g, '').trim() || ind.toLowerCase();
    return { keywords: ['protein gummies', 'pre workout gummies', 'electrolyte gummies', 'collagen gummies', base + ' powder', base + ' capsules', 'sports nutrition', 'whey protein', 'greens powder', 'magnesium gummies'], translated: {}, dry: true };
  }
...
// Reply sentiment: Positive (wants the video, a call, says yes), Negative (no, not interested, stop), Question (asks something), Neutral.
async function classifyReply(text, context) {
  const t = String(text || '').trim(); if (!t) return { sentiment: 'Neutral', summary: '' };
  const p = await aiJson('A prospect replied to a LinkedIn or email outreach offering a free sample product video' + (context && context.brand ? ' for the brand ' + context.brand : '') + '.\nReply:  + t.slice(0, 1500) + \nReturn {"sentiment": "Positive"|"Negative"|"Question"|"Neutral", "summary": one short sentence in British English saying what they want, no em dashes}', 'reply:' + t.slice(0, 200));
  if (p && ['Positive', 'Negative', 'Question', 'Neutral'].includes(p.sentiment)) return { sentiment: p.sentiment, summary: String(p.summary || '').slice(0, 200) };
  // no AI key: a plain keyword reading
  const neg = /\b(no thanks|not interested|no thank|stop|unsubscribe|remove me|not for us|pass)\b/i.test(t); const pos = /\b(yes|sure|send it|love to|sounds good|interested|go ahead|please do|let's|lets)\b/i.test(t);
  return { sentiment: neg ? 'Negative' : (pos ? 'Positive' : (/\?/.test(t) ? 'Question' : 'Neutral')), summary: '' };
}
// Ask AI (help assistant): answers from the guide text plus a live snapshot; flags FAQ-worthy questions and feature requests.
async function askAssistant(question, guideText, snapshot, history) {
  const q = String(question || '').trim(); if (!q) return null;
  const hist = (history || []).slice(0, 8).map((h) => 'Q: ' + h.question + '\nA: ' + String(h.answer || '').slice(0, 400)).join('\n\n');
  const prompt = 'You are the help assistant inside SitePounce Video Outreach, a tool that finds ecommerce brands advertising on Meta, scores them, sends LinkedIn connection requests, and tracks video outreach. Answer the user\'s question for a non-technical business owner. Rules: British English; short sentences; bullet points where steps are involved; refer to screens by their names as written in the guide (never invent web addresses); never use em dashes; if the guide does not cover it, say so plainly and suggest the nearest thing that exists. Use the live snapshot for anything about the user\'s own campaigns and numbers.\n\nGUIDE:\n' + String(guideText || '').slice(0, 14000) + '\n\nLIVE SNAPSHOT (JSON):\n' + JSON.stringify(snapshot || {}).slice(0, 4000) + (hist ? '\n\nEARLIER QUESTIONS FROM THIS USER:\n' + hist : '') +
    '\n\nQUESTION: ' + q.slice(0, 1500) +
    '\n\nReturn JSON: {"answer": the answer (plain text, blank lines between paragraphs, "- " for bullets), "faq_worthy": true if other users would plausibly ask this and the guide does not already answer it word for word, "faq_question": a short general wording of the question if faq_worthy, "faq_answer": a short general answer if faq_worthy (no user-specific numbers), "feature_request": if the user is asking for something the system cannot do, one sentence describing the feature, else ""}';
  const p = await aiJson(prompt, 'ask:' + crypto.createHash('sha1').update(q + '|' + (snapshot && snapshot.stamp || '')).digest('hex'));
  if (!p || !p.answer) return { answer: KEY.openai() ? 'The assistant could not answer just now. Try again in a moment.' : 'The AI key is not set, so the assistant is off. The Help and guide page covers the module in full.', faq_worthy: false, faq_question: '', faq_answer: '', feature_request: '' };
  return { answer: String(p.answer).replace(/\u2014/g, ', ').slice(0, 6000), faq_worthy: !!p.faq_worthy, faq_question: String(p.faq_question || '').slice(0, 300), faq_answer: String(p.faq_answer || '').replace(/\u2014/g, ', ').slice(0, 1500), feature_request: String(p.feature_request || '').slice(0, 500) };
}
...
async function featuredProducts(domain, products) {
  ...
  let text = '';
  try { const ctrl = new AbortController(); const t = setTimeout(() => ctrl.abort(), 5000); const r = await fetch('https://' + domain + '/', { signal: ctrl.signal, headers: { 'User-Agent': BROWSER_UA } }); clearTimeout(t); text = (await r.text()).replace(/<script[\s\S]*?<\/script>|<style[\s\S]*?<\/style>/gi, ' ').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').slice(0, 6000); } catch (e) { return []; }
...
module.exports = { debug, apifyStartRun, apifyStartPageCount, apifyPageAds, applyPageCounts, pageAdsUrl, apifyRunStatus, apifyDatasetItems, providerStatus, dryRun, COST, AI_MODEL, DEFAULT_ACTOR, fixtures, fixtureBrands, fixtureByDomain, keywordMatch, adLibraryUrl, normaliseAdRow, apifyMetaAds, groupAds, domainOf,
  ...
  shopifyProducts, wooProducts, mapWooProducts, storeProducts, hasStore, realPhotoCount, pickProduct, productsFromAds, INFOGRAPHIC, aiJson, suggestKeywords, analyseAds, heuristicAdAnalysis, featuredProducts, classifyReply, askAssistant, estimateRun };
