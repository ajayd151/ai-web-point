...
async function apolloOrgEnrich(domain) {
  ...
  return { employees: org.estimated_num_employees || null, shopify_plus: tech.some((t) => /shopify plus/.test(t)) ? true : null, linkedin_url: org.linkedin_url || '', technologies: tech, apollo_org_id: org.id || null, industry: org.industry || '', cost: COST.apolloCredit };
}
...
async function apolloPeople(domain, titles) {
  ...
  const r = await fetchRetry('https://api.apollo.io/api/v1/mixed_people/search', { method: 'POST', headers: apolloHeaders(), body: JSON.stringify({ q_organization_domains_list: [domain], person_titles: titles, page: 1, per_page: 5 }) }, { retries: 2, timeoutMs: 15000 });
  if (!r.ok) return { people: [], cost: COST.apolloCredit };
  const d = await r.json().catch(() => ({}));
  const people = (d.people || d.contacts || []).map((x) => ({ name: [x.first_name, x.last_name].filter(Boolean).join(' ') || x.name || '', title: x.title || '', linkedin: x.linkedin_url || '', email: x.email && !/email_not_unlocked/.test(x.email) ? x.email : '', email_status: x.email_status || '', apollo_id: x.id || null }));
...
async function apolloEmail(person) {
  ...
  return { email: p.email && !/email_not_unlocked/.test(p.email) ? p.email : '', status: p.email_status || '', cost: COST.apolloCredit };
}
...
function pickProduct(products, heroName, featuredHandles, opts) {
  ...
  const ranked = list.slice().sort((a, b) => (isHero(b) - isHero(a)) || (featIdx(a) - featIdx(b)) || (isNew(b) - isNew(a)) || ((b.reviews_count || 0) - (a.reviews_count || 0)) || (cleanScore(a.handle) - cleanScore(b.handle)));
  let weak = null; let tried = 0;
  for (const p of ranked) {
    tried++;
    const n = realPhotoCount(p);
    const reason = isHero(p) ? 'named most in the ads' : (featIdx(p) < 99 ? 'featured on the homepage' : (isNew(p) ? 'tagged as new or seasonal' : 'best remaining product'));
...
async function analyseAds(cand) {
  ...
  const out = { creative_gap: [0, 4, 8].includes(Number(p.creative_gap)) ? Number(p.creative_gap) : h.creative_gap,
    ...
    hero_product: String(p.hero_product || h.hero_product || '').slice(0, 120), observation: String(p.observation || '').trim(), style_reason: String(p.style_reason || '').slice(0, 200) };
...
module.exports = { providerStatus, dryRun, COST, AI_MODEL, DEFAULT_ACTOR, fixtures, fixtureBrands, fixtureByDomain, keywordMatch, adLibraryUrl, normaliseAdRow, apifyMetaAds, groupAds, domainOf,
