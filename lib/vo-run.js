...
async function processCandidate(cand, campaign, ctx) {
  ...
  if (!p.disqualified_reason) {
    ...
    else { const fromAds = S.productsFromAds(p.ad_samples, p.domain); if (fromAds.length) { p.products = fromAds; p.products_source = 'ads'; } else p.products_source = 'unknown'; p.skus = shop.skus; }
