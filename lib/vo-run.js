...
async function processCandidate(cand, campaign, ctx) {
  ...
  if (!p.disqualified_reason) {
    ...
    p.products = st.products.length ? st.products : null; p.products_source = st.source; p.skus = st.skus;
  ...
  } else { p.suggested_product_url = null; p.suggested_product_name = null; p.product_photo_check = 'Unverified'; p.why_this_product = p.products_source === 'unknown' ? 'No product found: their store blocks readers and their ads do not link to a product page. Type the product by hand.' : null; }
