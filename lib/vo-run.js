...
async function sourceCandidates(campaign, opts) {
  ...
  const allKw = keywords.concat(autoKw, translated, seeds.map((d) => d.split('.')[0]));
...
async function stepRun(store, campaign, run, opts) {
  ...
  if (!state.sourced) {
    ...
    if (!S.dryRun('apify')) {
      ...
      const kw = (Array.isArray(c.keywords) ? c.keywords : []).map(String).filter(Boolean).concat(Array.isArray(c.auto_keywords) ? c.auto_keywords.map(String).filter(Boolean) : [], c.keywords_translated && typeof c.keywords_translated === 'object' ? Object.values(c.keywords_translated).flat().map(String) : []);
      const rawCap = Math.max(10, Number(c.raw_cap) || 400);
      if (!state.apify) {
