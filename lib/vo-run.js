...
// Score first, spend second. Enrichment (Apollo: company record, job postings, people search, person match) can only add a
// bounded set of points: employees (best band), Shopify Plus, growth signals, a trigger event and a second contact with email.
// Everything else in the score is already known from the free Meta signals, the ad analysis and the store feed. So the best
// total a brand could reach after enrichment is computable before spending a credit; if that ceiling is under the threshold
// for the campaign's priority cut-off (default 4, Later), Apollo is skipped for the brand and it is parked with the reason.
function enrichmentCeiling(p, cfg) {
  return score(Object.assign({}, p, { employees: 11, shopify_plus: true, growth_signals: 99, trigger_event: true, second_contact_has_email: true, gatekeeper: false, disqualified_reason: '' }), cfg);
}
function enrichFloor(campaign, cfg) {
  const auto = (campaign && campaign.automation) || {};
  const n = Math.min(5, Math.max(1, Number(auto.enrich_floor || auto.max_priority) || 4));
  const name = Object.keys(cfg.priority_number || {}).find((k) => cfg.priority_number[k] === n) || 'Later';
  return { number: n, name: name, threshold: Number((cfg.priority || {})[name]) || 45 };
}
...
async function sourceCandidates(campaign, opts) {
  ...
  const autoKw = (Array.isArray(c.auto_keywords) ? c.auto_keywords : []).map(String).filter(Boolean);
  const topupKw = (Array.isArray(c.topup_keywords) ? c.topup_keywords : []).map(String).filter(Boolean);
  const allKw = o.topup && topupKw.length ? topupKw : keywords.concat(autoKw, translated, seeds.map((d) => d.split('.')[0]));
...
async function processCandidate(cand, campaign, ctx) {
  ...
  const p = { brand: cand.brand || cand.page_name || cand.domain, website: cand.website || cand.domain, domain: cand.domain, country: cand.country || (c.countries && c.countries[0]) || 'US',
    ...
    disqualified_reason: '', brand_instagram: '', apollo_org_id: cand.apollo_org_id || null, products: null, products_source: null, observation: '', ad_analysis: null, meta_page_id: cand.page_id || null };
  let cost = 0;
  ...
  if (!p.disqualified_reason) {
    ...
    const st = await S.storeProducts(p.domain, p.ad_samples, { dry: !!fx }); const shop = st.shop;
    p.products = st.products.length ? st.products : null; p.products_source = st.source; p.skus = st.skus; p._store_status = shop && shop.status;
    // spec 4.2: disqualify only when there is neither a products feed nor a cart. A store we cannot read is kept, products from the ads or by hand.
    if (!st.is_store && !fx && !(await S.hasStore(p.domain, shop))) disqualify('No DTC store detected (no products feed and no cart page)');
    else if (!st.is_store && !fx) p.trigger_note = (p.trigger_note ? p.trigger_note + '. ' : '') + 'Store is not on Shopify or WooCommerce (or blocks the products feed), product ' + (st.source === 'ads' ? 'taken from their ads' : 'picked by hand');
  }
  // score first, spend second (see enrichmentCeiling)
  let skipEnrich = false;
  if (!p.disqualified_reason && !fx && Number(p.active_meta_ads) >= 0) {
    const floor = enrichFloor(c, cfg); const ceil = enrichmentCeiling(p, cfg);
    if (floor.number < 5 && ceil.score_total != null && ceil.score_total < floor.threshold) {
      skipEnrich = true; p.dm_active_90d = 'Not found';
      p.notes = 'Company lookup skipped to save cost: even with the best company data this brand reaches ' + ceil.score_total + ' of the ' + floor.threshold + ' needed for ' + floor.name + ' (Meta signals too weak). Fill the Quick check fields by hand if you want it back.';
    }
  }
  if (!p.disqualified_reason && !skipEnrich) {
    // 4.4 enrichment
  ...
  if (p.products && p.products.length) {
    const featured = p.products_source === 'shopify' ? await S.featuredProducts(p.domain, p.products) : [];
    const pick = S.pickProduct(p.products, p.hero_product || '', featured, { source: p.products_source, keywords: (Array.isArray(c.keywords) ? c.keywords : []).concat(c.industry ? [c.industry] : []) });
    p.suggested_product_url = pick.url; p.suggested_product_name = pick.name; p.product_photo_check = pick.check; p.why_this_product = pick.why; p.product_candidates = pick.candidates || [];
  } else { p.suggested_product_url = null; p.suggested_product_name = null; p.product_photo_check = 'Unverified'; p.why_this_product = p.products_source === 'unknown' ? ((p._store_status === 403 || p._store_status === 401 || p._store_status === 429) ? 'Website blocked access (HTTP ' + p._store_status + '), so no product could be recommended. Type the product below or upload a photo of it.' : 'No product found: their store has no readable products feed and their ads do not link to a product page. Type the product below or upload a photo of it.') : null; }
  delete p._store_status;
  ...
  return { prospect: p, score: s2, messages: msgs, qualified: qualified, cost: cost, enrich_skipped: skipEnrich };
}
...
async function stepRun(store, campaign, run, opts) {
  ...
  if (!state.sourced) {
    ...
    if (!S.dryRun('apify')) {
      ...
      const topupKw = run.kind === 'top-up' && Array.isArray(c.topup_keywords) ? c.topup_keywords.map(String).filter(Boolean) : [];
      const kw = topupKw.length ? topupKw : (Array.isArray(c.keywords) ? c.keywords : []).map(String).filter(Boolean).concat(Array.isArray(c.auto_keywords) ? c.auto_keywords.map(String).filter(Boolean) : [], c.keywords_translated && typeof c.keywords_translated === 'object' ? Object.values(c.keywords_translated).flat().map(String) : []);
      const rawCap = Math.max(10, Number(c.raw_cap) || 400);
      if (!state.apify) {
    ...
    const src = await sourceCandidates(campaign, { topup: run.kind === 'top-up', existingDomains: await store.existingDomains(), globalExclusions: await store.globalExclusions(), today: o.today, apifyTimeoutSec: o.apifyTimeoutSec, adRowsByCountry: adRowsByCountry });
    ...
    if (!S.dryRun('apify') && src.candidates.length) {
      const pages = src.candidates.filter((c) => c.page_id && c.domain && !c.excluded_by && countryOk(c, campaign.countries)).map((c) => c.page_id);
      if (pages.length) {
        try {
  ...
  while (!stop) {
    ...
    try {
      ...
      if (r.enrich_skipped) state.counts.enrich_skipped = (state.counts.enrich_skipped || 0) + 1;
...
module.exports = { DEFAULT_EXCLUSIONS, brandFromDomain, sourceCandidates, processCandidate, stepRun, stopReason, newState, onExclusionList, growthFromSignals, enrichmentCeiling, enrichFloor };
