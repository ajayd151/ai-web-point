...
function tenantPrefix(req) {
  return tenantPrefixFor(accountEmailOf(req));
}
// Same thing from a bare email, for admin screens that report on someone else's workspace.
// NOTE: pass the WORKSPACE email. A team member shares their owner's prefix, so resolve them
// to the owner first (see teamPrefixFor in api/admin-limits.js).
function tenantPrefixFor(email) {
...
module.exports = { tenantPrefix, tenantPrefixFor, tenantKey, ownerEmail, emailOf, accountEmailOf, isTenantSlug, tenantSlug, ownsSlug };
