...
// Returns true ONLY when SendGrid actually accepted the message (2xx). Still never throws, because
// a mail problem must not break the request that triggered it, but callers can now tell the
// difference between sent and silently rejected. Anything that goes wrong is logged so a broken
// key or a suspended account shows up in the Vercel logs instead of vanishing.
async function sgSend(msg) {
  const key = process.env.SENDGRID_API_KEY;
  if (!key) { console.error('SendGrid: no API key set, email not sent'); return false; }
  try {
    ...
    if (r.ok) return true;
    let detail = '';
    try { detail = String(await r.text() || '').slice(0, 400); } catch (e) { /* body is optional */ }
    console.error('SendGrid rejected the message: ' + r.status + ' ' + detail);
    return false;
  } catch (e) {
    console.error('SendGrid send failed: ' + ((e && e.message) || e));
    return false;
  }
}
...
// `note` = what to do next, typed by the owner when clicking "Done & notify". It goes ABOVE the
// quoted feedback and is deliberately the loudest thing in the email: telling someone their bug is
// fixed is little use if they do not know they need to hard-refresh to see it.
async function sendFeedbackDoneEmail({ to, message, url, note }) {
  if (!process.env.SENDGRID_API_KEY || !to) return;
  const fromEmail = process.env.SITEPOUNCE_FROM_EMAIL || process.env.ERROR_EMAIL_FROM;
  if (!fromEmail) return;
  const base = process.env.APP_BASE_URL || 'https://sitepounce.com';
  ...
  const html = '<div style="font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#15233B;line-height:1.6;font-size:15px;">' +
    ...
    (n ? ('<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:14px 0;">' +
      '<tr><td style="background:#FFF8EC;border:1px solid #F3D9A8;border-left:5px solid #E0913A;border-radius:10px;padding:14px 16px;">' +
      '<div style="font-size:13px;font-weight:800;color:#15233B;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;">👉 What to do next</div>' +
      '<div style="font-size:15px;color:#15233B;line-height:1.6;white-space:pre-wrap;"><b>' + esc(n) + '</b></div>' +
      '</td></tr></table>') : '') +
...
function digestHtml(name, d, base) {
  ...
  let h = '<div style="background:#F4F7FA;padding:20px 0;font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;">' +
    ...
    tile('Appointments booked', d.meetingsBooked, moveChip(d.meetingsBooked, d.prevMeetingsBooked, w.cname), d.meetingsBooked ? '#EAFBF9' : '#F4F7FA') +
    '</tr></table></td></tr>';
  ...
  // how the day was shaped, and what it turned into
  if (d.status && d.status.rows.length) {
    h += '<tr><td style="padding:14px 22px 4px;">' +
      '<div style="font-size:13px;font-weight:800;color:' + D_NAVY + ';text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px;">Where they went</div>' +
      '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ' + D_LINE + ';border-radius:12px;">';
    d.status.rows.forEach((r, i) => {
      h += '<tr style="background:' + (i % 2 ? '#FBFCFE' : '#fff') + ';">' +
        '<td style="padding:9px 14px;font-size:14px;color:' + D_NAVY + ';border-top:' + (i ? '1px solid ' + D_LINE : '0') + ';">' + esc(r.label) + '</td>' +
        '<td align="right" style="padding:9px 14px;font-size:16px;font-weight:800;color:' + D_NAVY + ';border-top:' + (i ? '1px solid ' + D_LINE : '0') + ';">' + r.n + '</td>' +
        '</tr>';
    });
    h += '</table>' +
      '<div style="font-size:13px;color:' + D_MUTED + ';line-height:1.6;margin-top:8px;">' + esc(d.status.line) + '</div>' +
      '</td></tr>';
  }
  if (d.observations && d.observations.length) {
    h += '<tr><td style="padding:14px 22px 4px;">' +
      '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F4F7FA;border-radius:12px;">' +
      '<tr><td style="padding:14px 16px;">' +
      '<div style="font-size:13px;font-weight:800;color:' + D_NAVY + ';text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px;">A couple of observations</div>' +
      d.observations.map((o) => '<div style="font-size:13px;color:' + D_NAVY + ';line-height:1.6;margin-bottom:6px;">' +
        '<span style="color:' + D_TEAL + ';font-weight:800;">&bull;</span> ' + esc(o) + '</div>').join('') +
      '</td></tr></table></td></tr>';
  }
  ...
  if (ins && (ins.found.length || ins.advice.length || ins.objections.length)) {
    ...
    if (ins.advice.length) {
      ...
      ins.advice.forEach((a, i) => {
        h += '<div style="margin-bottom:13px;">' +
          ...
          (a.nextStep ? '<div style="font-size:13px;color:' + D_NAVY + ';line-height:1.6;margin-top:5px;background:#fff;border-radius:7px;padding:7px 10px;"><b style="color:' + D_TEAL + ';">Next step:</b> ' + esc(a.nextStep) + '</div>' : '') +
          '</div>';
      });
    }
    if (ins.objections.length) {
      ...
      ins.objections.forEach((o) => {
        h += '<div style="margin-bottom:12px;padding-left:10px;border-left:3px solid ' + D_TEAL + ';">' +
          ...
          (o.ask ? '<div style="font-size:13px;color:' + D_NAVY + ';line-height:1.6;margin-top:4px;"><b style="color:' + D_TEAL + ';">Then ask:</b> &ldquo;' + esc(o.ask) + '&rdquo;</div>' : '') +
          '</div>';
      });
    }
    h += '</td></tr></table></td></tr>';
...
function digestText(name, d) {
  ...
  let t = 'DAILY INSIGHTS\n\n' +
    ...
    'Appointments booked: ' + d.meetingsBooked + ' (' + w.cname + ': ' + d.prevMeetingsBooked + ')\n';
  if (d.rows && d.rows.length) {
    t += '\nWhat you did:\n';
  ...
  if (d.status && d.status.rows.length) {
    t += '\nWhere they went:\n';
    d.status.rows.forEach((r) => { t += '- ' + r.label + ': ' + r.n + '\n'; });
    t += d.status.line + '\n';
  }
  if (d.observations && d.observations.length) {
    t += '\nA couple of observations:\n';
    d.observations.forEach((o) => { t += '- ' + o + '\n'; });
  }
  const ins = d.insights;
  ...
  if (ins && ins.advice.length) {
    ...
    ins.advice.forEach((a, i) => {
      ...
      if (a.nextStep) t += '   Next step: ' + a.nextStep + '\n';
    });
  }
  if (ins && ins.objections.length) {
    t += '\nObjections you heard, and how to handle them:\n';
    ins.objections.forEach((o) => {
      t += '- "' + (o.objection || '') + '" -> ' + (o.handling || '') + '\n';
      if (o.ask) t += '  Then ask: "' + o.ask + '"\n';
    });
  }
...
async function sendDailyDigestEmail({ to, firstName, digest }) {
  ...
  const base = process.env.APP_BASE_URL || 'https://sitepounce.com';
  // Report what actually happened, so a failed send is retried by the catch-up run rather than
  // being marked as delivered and never seen again.
  return await sgSend({
    personalizations: [{ to: [{ email: to }] }],
    from: { email: fromEmail, name: 'Site Pounce' },
