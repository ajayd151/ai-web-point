...
function digestHtml(name, d, base) {
  ...
  if (d.insights && (d.insights.insights.length || d.insights.objections.length)) {
    h += '<tr><td style="padding:18px 22px 4px;">' +
      ...
      '<div style="font-size:15px;font-weight:800;color:' + D_NAVY + ';margin-bottom:10px;">Your top insights, from your own notes</div>';
    d.insights.insights.forEach((it, i) => {
      h += '<div style="margin-bottom:12px;">' +
        '<div style="font-size:14px;font-weight:700;color:' + D_NAVY + ';line-height:1.5;">' + (i + 1) + '. ' + esc(it.insight || '') + '</div>' +
        (it.suggestion ? '<div style="font-size:13px;color:' + D_MUTED + ';line-height:1.6;margin-top:3px;"><b style="color:' + D_TEAL + ';">Try today:</b> ' + esc(it.suggestion) + '</div>' : '') +
        '</div>';
    });
    if (d.insights.objections.length) {
      h += '<div style="font-size:14px;font-weight:800;color:' + D_NAVY + ';margin:14px 0 8px;">Objections you heard, and how to handle them</div>';
...
function digestText(name, d) {
  ...
  if (d.insights && d.insights.insights.length) {
    t += '\nYour top insights, from your own notes:\n';
    d.insights.insights.forEach((it, i) => {
      t += (i + 1) + '. ' + (it.insight || '') + '\n';
      if (it.suggestion) t += '   Try today: ' + it.suggestion + '\n';
    });
  }
  if (d.insights && d.insights.objections.length) {
    t += '\nObjections you heard, and how to handle them:\n';
    d.insights.objections.forEach((o) => { t += '- "' + (o.objection || '') + '" -> ' + (o.handling || '') + '\n'; });
  }
