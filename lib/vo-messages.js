...
const CLOSING_CHAT = 'If you like it, I can make more of these for you every day, or every week if you prefer, so you always have fresh ads to test. Would you be open to a quick chat about that? Tell me a day and time that suits and I will set it up.';
...
function messageA(ctx) {
  ...
  return joinParas([
    'Hey ' + first,
    'I run ' + pr.service_name + '. Came across ' + ctx.observation + (ctx.proof || '') + ', so I made you a free sample video for ' + ctx.product + ': ' + (ctx.video_url || URL_PLACEHOLDER),
    CLOSING_CHAT,
    pr.signoff + ',',
    senderLine(pr),
  ]);
...
const DEFAULT_TEMPLATES = {
  ...
  message_a: 'Hey {first}\n\nI run {service_name}. Came across {observation}{proof}, so I made you a free sample video for {product}: {video_url}\n\n{closing_question}\n\n{signoff},\n\n{sender_line}',
...
module.exports = { CLOSING_CHAT, forAttachment, forLink, ATTACHED, softNote, NOTE_VARIANTS, DEFAULT_PROFILE, DEFAULT_TEMPLATES, render, templatesWith, URL_PLACEHOLDER, firstName, cleanObservation, genericObservation, proofLine, countsTrusted, nextProduct, shortProduct, connectionNote, messageA, messageB, followUp1, followUp2, postCheck, generate };
