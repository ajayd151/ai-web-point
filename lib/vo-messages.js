...
// One proof line built from real numbers, so the message shows we looked. Blank when we have nothing solid.
function proofLine(p) {
  ...
  if (n30 >= 5) return 'You have put ' + n30 + ' new ads live this month, so you clearly need a steady flow of creative.';
  if (active >= 30) return 'You are running ' + active + ' ads right now, which takes a lot of creative to keep fresh.';
  if (style === 'Static' && active > 0) return 'Most of them are still images, and video tends to work harder for a product like this.';
  return '';
}
...
function shortProduct(title) {
  ...
  const spec = /(\d+(\.\d+)?\s?(g|mg|ml|oz|lb|kg|ct|pack|packs|count|servings?|capsules?|gummies each|per\b)|\bper\s+\w+|\b\d+\s*x\b|bags? of|bottles? of|\d+-pack)/i;
  const parts = t.split(/\s+-\s+|\s+\u2013\s+|\s*\(|\)\s*/).map((x) => x.trim()).filter(Boolean).filter((x) => !spec.test(x));
  t = (parts.length ? parts.join(' ') : t).replace(/\s+/g, ' ').trim();
  t = t.replace(/\s+\d+(\.\d+)?\s?(g|mg|ml|oz)\b.*$/i, '').trim();
  const words = t.split(' '); if (words.length > 7) t = words.slice(0, 7).join(' ');
...
function messageA(ctx) {
  ...
  return joinParas([
    'Hey ' + first,
    'I run ' + pr.service_name + '. Came across ' + ctx.observation + '.' + (ctx.proof ? ' ' + ctx.proof : ''),
    'So I made you a free sample for ' + ctx.product + ': ' + (ctx.video_url || URL_PLACEHOLDER),
    ctx.next_product ? 'If it is any good, want me to do one for ' + ctx.next_product + ' next?' : 'Worth a look?',
    pr.signoff + ',',
    senderLine(pr),
  ]);
}
...
function render(template, ctx) {
  ...
  const vars = { first: ctx.first || firstName(ctx.dm_name), service_name: pr.service_name, service_desc: pr.service_desc, observation: ctx.observation || '', product: ctx.product || 'your hero product',
    video_url: ctx.video_url || URL_PLACEHOLDER, signoff: pr.signoff, sender_first: pr.sender_first, sender_title: pr.sender_title || '', sender_line: senderLine(pr), signature: senderLine(pr), proof: ctx.proof || '', next_product: ctx.next_product || '', closing_question: ctx.next_product ? 'If it is any good, want me to do one for ' + ctx.next_product + ' next?' : 'Worth a look?', offer_line: pr.offer_line, pilot_line: pr.pilot_line, seasonal_event: ctx.seasonal_event || 'the season ahead' };
...
module.exports = { DEFAULT_PROFILE, DEFAULT_TEMPLATES, render, templatesWith, URL_PLACEHOLDER, firstName, cleanObservation, genericObservation, proofLine, nextProduct, shortProduct, connectionNote, messageA, messageB, followUp1, followUp2, postCheck, generate };
