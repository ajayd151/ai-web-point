...
// {sender_first} {offer_line} {pilot_line} {seasonal_event}. A blank line (or " / ") separates paragraphs.
...
function render(template, ctx) {
  ...
  const vars = { first: ctx.first || firstName(ctx.dm_name), service_name: pr.service_name, service_desc: pr.service_desc, observation: ctx.observation || '', product: ctx.product || 'your hero product',
    video_url: ctx.video_url || URL_PLACEHOLDER, signoff: pr.signoff, sender_first: pr.sender_first, offer_line: pr.offer_line, pilot_line: pr.pilot_line, seasonal_event: ctx.seasonal_event || 'the season ahead' };
...
module.exports = { DEFAULT_PROFILE, DEFAULT_TEMPLATES, render, templatesWith, URL_PLACEHOLDER, firstName, genericObservation, connectionNote, messageA, messageB, followUp1, followUp2, postCheck, generate };
