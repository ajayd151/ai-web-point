...
const ATTACHED = ', it is attached below';
...
function messageA(ctx) {
  ...
  return joinParas([
    'Hey ' + first,
    'I run ' + pr.service_name + '. Came across ' + ctx.observation + (ctx.proof || '') + ', so I made you a free sample for ' + ctx.product + ': ' + (ctx.video_url || URL_PLACEHOLDER),
...
const DEFAULT_TEMPLATES = {
  ...
  message_a: 'Hey {first}\n\nI run {service_name}. Came across {observation}{proof}, so I made you a free sample for {product}: {video_url}\n\n{closing_question}\n\n{signoff},\n\n{sender_line}',
...
module.exports = { forAttachment, forLink, ATTACHED, softNote, NOTE_VARIANTS, DEFAULT_PROFILE, DEFAULT_TEMPLATES, render, templatesWith, URL_PLACEHOLDER, firstName, cleanObservation, genericObservation, proofLine, countsTrusted, nextProduct, shortProduct, connectionNote, messageA, messageB, followUp1, followUp2, postCheck, generate };
