...
// The season hook for Follow-up 3, worked out at the moment of sending (the brands are US ones, so US dates).
...
function usOccasions(y) {
  return [
    { name: "Valentine's Day", major: true, date: new Date(Date.UTC(y, 1, 14)) },
    ...
    { name: 'the 4th of July', major: false, date: new Date(Date.UTC(y, 6, 4)) },
    { name: 'Labor Day', major: false, date: nthWeekday(y, 9, 1, 1) },
    { name: 'Halloween', major: false, date: new Date(Date.UTC(y, 9, 31)) },
    { name: 'Thanksgiving and Black Friday', major: true, date: nthWeekday(y, 11, 4, 4) },
    { name: 'Christmas', major: true, date: new Date(Date.UTC(y, 11, 25)) },
    { name: 'the new year', major: true, date: new Date(Date.UTC(y + 1, 0, 1)) },
  ];
}
function upcomingOccasion(now) {
  const t = now || new Date(); const y = t.getUTCFullYear();
  const all = usOccasions(y - 1).concat(usOccasions(y), usOccasions(y + 1)).map((o) => Object.assign(o, { days: Math.floor((o.date - t) / 86400000) }));
...
function seasonalHook(now) {
  const o = upcomingOccasion(now);
...
function followUp3(ctx) {
  ...
  return joinParas([
    'Hey ' + first,
    seasonalHook(ctx.now) + ', most brands we talk to are short on fresh video ads. If you want, I can make a second sample for ' + (ctx.next_product || 'another of your products') + ', free as before.',
  ]);
}
...
module.exports = { seasonalHook, upcomingOccasion, CLOSING_CHAT, forAttachment, forLink, ATTACHED, softNote, NOTE_VARIANTS, DEFAULT_PROFILE, DEFAULT_TEMPLATES, render, templatesWith, URL_PLACEHOLDER, firstName, cleanObservation, genericObservation, proofLine, countsTrusted, nextProduct, shortProduct, connectionNote, messageA, messageB, followUp1, followUp2, postCheck, generate };
