...
// The season hook for Follow-up 3, so a message a month later has a real reason to exist.
function seasonalHook(now) {
  const d = now || new Date(); const m = d.getUTCMonth() + 1, day = d.getUTCDate();
  if ((m === 9 && day >= 15) || m === 10 || (m === 11 && day <= 27)) return 'With Black Friday coming';
  if ((m === 11 && day > 27) || (m === 12 && day <= 20)) return 'With Christmas orders peaking';
  if ((m === 12 && day > 20) || m === 1) return 'With the new year push starting';
  if (m === 2 || m === 3) return 'With spring launches coming up';
  if (m >= 4 && m <= 6) return 'With summer coming';
  return 'With Q4 planning starting';
}
...
module.exports = { seasonalHook, CLOSING_CHAT, forAttachment, forLink, ATTACHED, softNote, NOTE_VARIANTS, DEFAULT_PROFILE, DEFAULT_TEMPLATES, render, templatesWith, URL_PLACEHOLDER, firstName, cleanObservation, genericObservation, proofLine, countsTrusted, nextProduct, shortProduct, connectionNote, messageA, messageB, followUp1, followUp2, postCheck, generate };
