...
// it was about (business name/slug) so we can count UNIQUE businesses. Fails soft.
async function logActivity(actor, account, action, detail, subject) {
  if (!dbConfigured() || !actor || !action) return false;
  try {
    await ensureActivity();
    await sql`INSERT INTO activity_log (actor, account, action, detail, subject)
...
async function activityReport(actor, from, to) {
  ...
  try {
    ...
    const barows = (await sql`SELECT COALESCE(SUM(((regexp_match(detail, '\\((\\d+)\\)$'))[1])::int), 0)::int AS n
