...
async function dashboardData(since) {
  ...
  const counts = (await sql`SELECT event, COUNT(DISTINCT slug)::int AS slugs, COUNT(*)::int AS total FROM link_events WHERE slug NOT LIKE 'dm-%' AND (${s}::timestamptz IS NULL OR ts >= ${s}::timestamptz) GROUP BY event`).rows;
  const channel = (await sql`SELECT event, platform, COUNT(DISTINCT slug)::int AS slugs FROM link_events WHERE event IN ('sent','view') AND platform IS NOT NULL AND platform <> '' AND slug NOT LIKE 'dm-%' AND (${s}::timestamptz IS NULL OR ts >= ${s}::timestamptz) GROUP BY event, platform`).rows;
  const byHour = (await sql`SELECT EXTRACT(HOUR FROM ts AT TIME ZONE 'Europe/London')::int AS h, COUNT(*)::int AS n FROM link_events WHERE event='view' AND slug NOT LIKE 'dm-%' AND (${s}::timestamptz IS NULL OR ts >= ${s}::timestamptz) GROUP BY h`).rows;
  const byDow = (await sql`SELECT EXTRACT(DOW FROM ts AT TIME ZONE 'Europe/London')::int AS d, COUNT(*)::int AS n FROM link_events WHERE event='view' AND slug NOT LIKE 'dm-%' AND (${s}::timestamptz IS NULL OR ts >= ${s}::timestamptz) GROUP BY d`).rows;
  const ttoRows = (await sql`
    ...
    FROM (
      ...
      FROM link_events WHERE slug NOT LIKE 'dm-%' AND (${s}::timestamptz IS NULL OR ts >= ${s}::timestamptz) GROUP BY slug
  ...
  const rows = (await sql`
    ...
    FROM link_events WHERE slug NOT LIKE 'dm-%' AND (${s}::timestamptz IS NULL OR ts >= ${s}::timestamptz) GROUP BY slug ORDER BY MAX(ts) DESC LIMIT 100`).rows;
  ...
  const byDay = (await sql`
    ...
    FROM link_events
    WHERE ts >= (now() - interval '31 days') AND slug NOT LIKE 'dm-%'
  ...
  const byTemplate = (await sql`
    ...
    FROM link_events
    WHERE tpl IS NOT NULL AND tpl <> '' AND (${s}::timestamptz IS NULL OR ts >= ${s}::timestamptz)
...
module.exports = { recordEvent, statsBySlug, dbConfigured, recordApplication, dashboardData, hotLeadRows, getUserByEmail, upsertUser, markWelcomed };
