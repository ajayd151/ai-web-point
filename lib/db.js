...
// Most-recent feedback for the Super Admin review (owner-only endpoint).
async function feedbackList(limit) {
  if (!dbConfigured()) return [];
  try {
    await ensureFeedback();
    const n = Math.min(Math.max(Number(limit) || 100, 1), 500);
    const { rows } = await sql`SELECT id, created, email, plan, status, type, importance, message, page, url, handled
      FROM feedback ORDER BY created DESC LIMIT ${n}`;
    return rows;
  } catch (e) { console.error('feedbackList failed:', e.message); return []; }
}

module.exports = { recordEvent, statsBySlug, dbConfigured, recordApplication, dashboardData, hotLeadRows, getUserByEmail, upsertUser, markWelcomed, recordFeedback, feedbackList };
