...
// events still count). `search` is excluded from "businesses worked".
async function activityReport(actor, from, to) {
  ...
  try {
    ...
    const urows = (await sql`SELECT COUNT(DISTINCT COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)))::int AS n
      FROM activity_log WHERE actor = ${a} AND action <> 'search' AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz)`).rows;
    const grows = (await sql`WITH firsts AS (
        ...
        FROM activity_log WHERE actor = ${a} AND action <> 'search' AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz) GROUP BY 1
