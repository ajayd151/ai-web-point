...
async function activityReport(actor, days) {
  ...
  try {
    ...
    const counts = (await sql`SELECT action, COUNT(*)::int AS n,
      ...
      FROM activity_log WHERE actor = ${a} AND (${since}::text IS NULL OR ts >= now() - (${since})::interval) GROUP BY action`).rows;
    const urows = (await sql`SELECT COUNT(DISTINCT COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)))::int AS n
      FROM activity_log WHERE actor = ${a} AND action <> 'search' AND (${since}::text IS NULL OR ts >= now() - (${since})::interval)`).rows;
    const grows = (await sql`WITH firsts AS (
        ...
        FROM activity_log WHERE actor = ${a} AND action <> 'search' AND (${since}::text IS NULL OR ts >= now() - (${since})::interval) GROUP BY 1
    ...
    const mrows = (await sql`SELECT COUNT(DISTINCT COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)))::int AS n
      FROM activity_log WHERE actor = ${a} AND action = 'status_update' AND detail LIKE '% → meeting-booked%'
        AND (${since}::text IS NULL OR ts >= now() - (${since})::interval)`).rows;
