...
async function ensureTable() {
  ...
  } catch (e) { /* migrations are IF NOT EXISTS; a transient lock on a re-run must not make reads return empty. Tables exist from prior deploys. */ }
  ensured = true;
}
...
async function feedbackList(limit, status) {
  ...
  try {
    ...
    const { rows } = await sql`SELECT id, created, email, plan, status AS acct_status, type, importance, message, page, url, ua, admin_status
...
// One feedback row (for the 'notify the submitter it is done' email).
async function getFeedbackById(id) {
  if (!dbConfigured() || !id) return null;
  try {
    await ensureFeedback();
    const { rows } = await sql`SELECT id, email, message, url, page, type FROM feedback WHERE id = ${Number(id)} LIMIT 1`;
    return rows[0] || null;
  } catch (e) { return null; }
}
...
async function ensureActivity() {
  ...
  await sql`ALTER TABLE activity_log ADD COLUMN IF NOT EXISTS subject TEXT`; // the business it was about (for unique counts)
  await sql`CREATE INDEX IF NOT EXISTS activity_actor_idx ON activity_log (actor, ts DESC)`;
  } catch (e) { /* migrations are IF NOT EXISTS; a transient lock on a re-run must not make reads return empty. Tables exist from prior deploys. */ }
  activityEnsured = true;
}
// Record one action. actor = who did it, account = the workspace owner, subject = the business
// it was about (business name/slug) so we can count UNIQUE businesses. `meta` is optional
// structured extra (e.g. { added, skipped } on a call-list batch) for things a text detail cannot
// hold reliably. Fails soft.
async function logActivity(actor, account, action, detail, subject, meta) {
  if (!dbConfigured() || !actor || !action) return false;
  try {
    await ensureActivity();
    const m = (meta && typeof meta === 'object') ? JSON.stringify(meta) : null;
    await sql`INSERT INTO activity_log (actor, account, action, detail, subject, meta)
      VALUES (${String(actor).toLowerCase()}, ${account ? String(account).toLowerCase() : null}, ${action},
              ${detail ? String(detail).slice(0, 300) : null}, ${subject ? String(subject).slice(0, 160) : null},
              ${m}::jsonb)`;
    return true;
  } catch (e) { return false; }
}
// Aggregated report for one person. Includes UNIQUE businesses (distinct prospects) overall and
// per action, and the average gap between first working one business and the next.
// `bizkey` = the subject if recorded, else the business name parsed out of the detail (so old
// events still count).
//
// EXCLUDED from "businesses worked": 'search' (not about one business), and 'call_add'/'call_remove'
// (one row covers a whole BATCH, so its detail is a list like "A, B, C +41 more (46)". Parsing that
// as a single business name would add one junk entry per batch and inflate the unique count).
//
// `businessesAdded` is the real number of businesses put on the call list: each call_add row ends
// with its batch size in brackets, e.g. "(46)", so we sum that rather than counting rows. Counting
// rows would report the number of times someone clicked Add, which is not what "Calls added" means.
async function activityReport(actor, from, to) {
  if (!dbConfigured() || !actor) return null;
  try {
    await ensureActivity();
    const a = String(actor).toLowerCase();
    const f = from || null; const t = to || null; // ISO timestamps or null (open-ended)
    const counts = (await sql`SELECT action, COUNT(*)::int AS n,
        COUNT(DISTINCT CASE WHEN action <> 'search' THEN COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)) END)::int AS uniq
      FROM activity_log WHERE actor = ${a} AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz) GROUP BY action`).rows;
    const urows = (await sql`SELECT COUNT(DISTINCT COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)))::int AS n
      FROM activity_log WHERE actor = ${a} AND action NOT IN ('search','call_add','call_remove') AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz)`).rows;
    const grows = (await sql`WITH firsts AS (
        SELECT COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)) AS bizkey, MIN(ts) AS ft
        FROM activity_log WHERE actor = ${a} AND action NOT IN ('search','call_add','call_remove') AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz) GROUP BY 1
      ), ordered AS (SELECT ft, LAG(ft) OVER (ORDER BY ft) AS prev FROM firsts)
      SELECT AVG(EXTRACT(EPOCH FROM (ft - prev))) / 60.0 AS avg FROM ordered WHERE prev IS NOT NULL`).rows;
    // real businesses added to the call list: sum each batch's size from the "(46)" at the end of
    // its detail, instead of counting one row per batch
    const barows = (await sql`SELECT
        COALESCE(SUM(((regexp_match(detail, '\\((\\d+)\\)$'))[1])::int), 0)::int AS n,
        COALESCE(SUM((meta->>'skipped')::int), 0)::int AS skipped
      FROM activity_log WHERE actor = ${a} AND action = 'call_add'
        AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz)`).rows;
    // unique businesses this person moved to "Appointment booked" (value stays 'meeting-booked')
    const mrows = (await sql`SELECT COUNT(DISTINCT COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)))::int AS n
      FROM activity_log WHERE actor = ${a} AND action = 'status_update' AND detail LIKE '% → meeting-booked%'
        AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz)`).rows;
    // machine-made mockups (SMS campaigns), so reporting can split man vs machine
    const amrows = (await sql`SELECT COUNT(*)::int AS n FROM activity_log
      WHERE actor = ${a} AND action = 'mockup' AND (meta->>'auto' = '1' OR detail LIKE '%(auto)')
        AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz)`).rows;
    // Prowls split Mini (Google only, no AI credit) vs Full, for cost tracking
    const mprows = (await sql`SELECT COUNT(*)::int AS n FROM activity_log
      WHERE actor = ${a} AND action = 'prowl' AND (meta->>'mini' = '1' OR detail LIKE '%(mini)')
        AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz)`).rows;
    // unique businesses this person sent an appointment link to
    const lrows = (await sql`SELECT COUNT(DISTINCT COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)))::int AS n
      FROM activity_log WHERE actor = ${a} AND action = 'status_update' AND detail LIKE '% → appointment-link-sent%'
        AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz)`).rows;
    const recent = (await sql`SELECT ts, action, detail FROM activity_log
      WHERE actor = ${a} AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz) ORDER BY ts DESC LIMIT 100`).rows;
    // Hour-by-hour and day-by-day shape of the work, for spotting the gaps. Hours are London
    // local, not UTC, or the whole picture would be shifted an hour in summer.
    // `uniq` uses the same definition as the headline unique count (searches and call-list
    // batches are not one identifiable business, so they cannot count as a business worked).
    const hrows = (await sql`SELECT EXTRACT(HOUR FROM ts AT TIME ZONE 'Europe/London')::int AS hour,
        COUNT(*)::int AS n,
        COUNT(DISTINCT CASE WHEN action NOT IN ('search','call_add','call_remove')
          THEN COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)) END)::int AS uniq,
        SUM(CASE WHEN action = 'status_update' THEN 1 ELSE 0 END)::int AS updates
      FROM activity_log WHERE actor = ${a}
        AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz)
      GROUP BY 1 ORDER BY 1`).rows;
    // Which statuses they moved businesses to. The status is the bit after the arrow in the
    // detail, e.g. "Bela Lash & Brow → meeting-booked (note added)".
    const strows = (await sql`SELECT (regexp_match(detail, ' → ([a-z-]+)'))[1] AS status,
        COUNT(*)::int AS n,
        COUNT(DISTINCT COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)))::int AS uniq
      FROM activity_log WHERE actor = ${a} AND action = 'status_update' AND detail ~ ' → [a-z-]+'
        AND (${f}::timestamptz IS NULL OR ts >= ${f}::timestamptz) AND (${t}::timestamptz IS NULL OR ts < ${t}::timestamptz)
      GROUP BY 1 ORDER BY 2 DESC`).rows;
    const drows = (await sql`SELECT to_char(ts AT TIME ZONE 'Europe/London', 'YYYY-MM-DD') AS day,
        COUNT(*)::int AS n,
        COUNT(DISTINCT CASE WHEN action NOT IN ('search','call_add','call_remove')
          THEN COALESCE(NULLIF(subject,''), split_part(split_part(detail, ' → ', 1), ' (', 1)) END)::int AS uniq
      FROM activity_log WHERE actor = ${a}
    ...
    return {
      counts: counts,
      uniqueBusinesses: (urows[0] && urows[0].n) || 0,
      avgGapMin: (grows[0] && grows[0].avg != null) ? Number(grows[0].avg) : null,
      meetingsBooked: (mrows[0] && mrows[0].n) || 0,
      linksSent: (lrows[0] && lrows[0].n) || 0,
      businessesAdded: (barows[0] && barows[0].n) || 0,
      mockupsAuto: (amrows[0] && amrows[0].n) || 0,
      prowlsMini: (mprows[0] && mprows[0].n) || 0,
      duplicatesSkipped: (barows[0] && barows[0].skipped) || 0,
      byHour: hrows,
      byDay: drows,
      byStatus: strows,
      recent: recent,
...
// Everyone who did something in a window. Drives the 8am digest (we only email people who worked).
async function activeActors(from, to) {
  if (!dbConfigured()) return [];
  try {
    await ensureActivity();
    const { rows } = await sql`SELECT actor, COUNT(*)::int AS n FROM activity_log
      WHERE ts >= ${from}::timestamptz AND ts < ${to}::timestamptz AND actor <> ''
      GROUP BY actor ORDER BY n DESC`;
    return rows;
  } catch (e) { return []; }
}
// First and last activity plus the total for one person in a window (the "you started at / finished at").
async function activitySpan(actor, from, to) {
  if (!dbConfigured() || !actor) return null;
  try {
    await ensureActivity();
    const { rows } = await sql`SELECT MIN(ts) AS first_ts, MAX(ts) AS last_ts, COUNT(*)::int AS n
      FROM activity_log WHERE actor = ${String(actor).toLowerCase()}
        AND ts >= ${from}::timestamptz AND ts < ${to}::timestamptz`;
    const r = rows[0] || {};
    return { first: r.first_ts || null, last: r.last_ts || null, total: r.n || 0 };
  } catch (e) { return null; }
}
...
// ---- per-person rate limits, editable in Admin > Limits ----
// One row per person. `limits` holds only the kinds that have been overridden, e.g.
// {"search":200}. Anything absent falls back to the env default (LIMIT_SEARCH etc), so a blank
// box in Admin means "use the default" rather than "zero".
let rateLimitsEnsured = false;
async function ensureRateLimits() {
  if (rateLimitsEnsured) return;
  try {
  await sql`CREATE TABLE IF NOT EXISTS rate_limits (
    email TEXT PRIMARY KEY, limits JSONB NOT NULL DEFAULT '{}'::jsonb,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
  )`;
  } catch (e) { /* migrations are IF NOT EXISTS; a transient lock on a re-run must not make reads return empty. Tables exist from prior deploys. */ }
  rateLimitsEnsured = true;
}
async function getRateLimits(email) {
  if (!dbConfigured() || !email) return null;
  try {
    await ensureRateLimits();
    const { rows } = await sql`SELECT limits FROM rate_limits WHERE email = ${String(email).toLowerCase()}`;
    return (rows[0] && rows[0].limits) || null;
  } catch (e) { return null; }
}
async function setRateLimits(email, limits) {
  if (!dbConfigured() || !email) return false;
  try {
    await ensureRateLimits();
    const j = JSON.stringify(limits || {});
    await sql`INSERT INTO rate_limits (email, limits, updated_at) VALUES (${String(email).toLowerCase()}, ${j}::jsonb, now())
      ON CONFLICT (email) DO UPDATE SET limits = ${j}::jsonb, updated_at = now()`;
    return true;
  } catch (e) { return false; }
}
async function listRateLimits() {
  ...
  try {
    await ensureRateLimits();
    const { rows } = await sql`SELECT email, limits FROM rate_limits`;
    return rows;
  } catch (e) { return []; }
}
...
async function ensureTeam() {
  ...
  } catch (e) { /* migrations are IF NOT EXISTS; a transient lock on a re-run must NOT make team
    reads return empty (which would strip members of access). The table exists from prior deploys. */ }
  teamEnsured = true;
}
// Owner-only diagnostic: is a member's row missing, is the read failing, or is it under a
// different owner_email? Returns raw rows or the actual error, so we can tell them apart.
async function teamDebug() {
  if (!dbConfigured()) return { db: false };
  try {
    await ensureTeam();
    const { rows } = await sql`SELECT owner_email, member_email, suspended, created_at FROM team_members ORDER BY created_at DESC LIMIT 25`;
    return { db: true, total: rows.length, rows: rows };
  } catch (e) { return { db: true, error: e.message }; }
}
...
module.exports = { recordEvent, statsBySlug, dbConfigured, recordApplication, dashboardData, hotLeadRows, getUserByEmail, upsertUser, markWelcomed, recordFeedback, feedbackList, setFeedbackStatus, deleteFeedback, getFeedbackById, recordDeepDossierRun, getTeamMember, teamDebug, listTeamMembers, addTeamMember, setTeamSuspended, setTeamPermissions, removeTeamMember, clearMustChange, feedbackCounts, countActiveCustomers, listUsers, getDailyUsage, bumpDailyUsage, logActivity, activityReport, activeActors, activitySpan, recordNote, notesLog, getRateLimits, setRateLimits, listRateLimits, saveDeepDossierLeads, listDeepDossierLeads };
