...
async function ensureFeedback() {
  ...
  await sql`ALTER TABLE feedback ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'new'`; // new | done | ignored
...
async function feedbackList(limit, status) {
  ...
  try {
    ...
    const { rows } = await sql`SELECT id, created, email, plan, status, type, importance, message, page, url
      FROM feedback WHERE (${st}::text IS NULL OR status = ${st})
...
async function setFeedbackStatus(id, status) {
  ...
  try {
    await ensureFeedback();
    const { rows } = await sql`UPDATE feedback SET status = ${st}, handled = ${st !== 'new'} WHERE id = ${Number(id)} RETURNING id`;
    return rows.length > 0;
...
module.exports = { recordEvent, statsBySlug, dbConfigured, recordApplication, dashboardData, hotLeadRows, getUserByEmail, upsertUser, markWelcomed, recordFeedback, feedbackList, setFeedbackStatus, deleteFeedback };
