...
async function getTeamMember(memberEmail) {
  ...
  try {
    await ensureTeam();
    const { rows } = await sql`SELECT owner_email, member_email, first_name, last_name, suspended, permissions, limits FROM team_members WHERE member_email = ${String(memberEmail).toLowerCase()} LIMIT 1`;
    return rows[0] || null;
...
async function addTeamMember(ownerEmail, memberEmail, firstName, lastName, permissions, limits) {
  ...
  try {
    await ensureTeam();
    await sql`INSERT INTO team_members (owner_email, member_email, first_name, last_name, suspended, permissions, limits)
      VALUES (${String(ownerEmail).toLowerCase()}, ${String(memberEmail).toLowerCase()}, ${firstName || null}, ${lastName || null}, false, ${perms}::jsonb, ${lims}::jsonb)
      ON CONFLICT (member_email) DO UPDATE SET owner_email = ${String(ownerEmail).toLowerCase()},
        first_name = ${firstName || null}, last_name = ${lastName || null}, suspended = false, permissions = ${perms}::jsonb, limits = ${lims}::jsonb`;
    return true;
