...
async function getTeamMember(memberEmail) {
  ...
  try {
    await ensureTeam();
    const { rows } = await sql`SELECT owner_email, member_email, first_name, last_name, suspended, permissions FROM team_members WHERE member_email = ${String(memberEmail).toLowerCase()} LIMIT 1`;
    return rows[0] || null;
...
async function listTeamMembers(ownerEmail) {
  ...
  try {
    await ensureTeam();
    const { rows } = await sql`SELECT member_email, first_name, last_name, suspended, permissions, created_at FROM team_members WHERE owner_email = ${String(ownerEmail).toLowerCase()} ORDER BY created_at DESC`;
    return rows;
  } catch (e) { console.error('listTeamMembers failed:', e.message); return []; }
}
async function addTeamMember(ownerEmail, memberEmail, firstName, lastName, permissions) {
  if (!dbConfigured() || !ownerEmail || !memberEmail) return false;
  const perms = JSON.stringify(permissions || {});
  try {
    await ensureTeam();
    await sql`INSERT INTO team_members (owner_email, member_email, first_name, last_name, suspended, permissions)
      VALUES (${String(ownerEmail).toLowerCase()}, ${String(memberEmail).toLowerCase()}, ${firstName || null}, ${lastName || null}, false, ${perms}::jsonb)
      ON CONFLICT (member_email) DO UPDATE SET owner_email = ${String(ownerEmail).toLowerCase()},
        first_name = ${firstName || null}, last_name = ${lastName || null}, suspended = false, permissions = ${perms}::jsonb`;
    return true;
  } catch (e) { console.error('addTeamMember failed:', e.message); return false; }
}
async function setTeamPermissions(ownerEmail, memberEmail, permissions) {
  if (!dbConfigured() || !memberEmail) return false;
  const perms = JSON.stringify(permissions || {});
  try {
    await ensureTeam();
    const { rows } = await sql`UPDATE team_members SET permissions = ${perms}::jsonb
