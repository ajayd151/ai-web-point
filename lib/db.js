...
async function dashboardData() {
  ...
  await ensureTable();
  const counts = (await sql`SELECT event, COUNT(DISTINCT slug)::int AS slugs, COUNT(*)::int AS total FROM link_events GROUP BY event`).rows;
  const channel = (await sql`SELECT event, platform, COUNT(DISTINCT slug)::int AS slugs FROM link_events WHERE event IN ('sent','view') AND platform IS NOT NULL AND platform <> '' GROUP BY event, platform`).rows;
  const byHour = (await sql`SELECT EXTRACT(HOUR FROM ts AT TIME ZONE 'Europe/London')::int AS h, COUNT(*)::int AS n FROM link_events WHERE event='view' GROUP BY h`).rows;
  const byDow = (await sql`SELECT EXTRACT(DOW FROM ts AT TIME ZONE 'Europe/London')::int AS d, COUNT(*)::int AS n FROM link_events WHERE event='view' GROUP BY d`).rows;
  ...
  const rows = (await sql`
    ...
    FROM link_events GROUP BY slug ORDER BY MAX(ts) DESC LIMIT 100`).rows;
