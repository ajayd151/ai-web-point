...
function confidence(row, verify, ch) {
  ...
  if (row.linkedin) s += 8;
  if (row.email) s += 8;
  if (verify.verified) s += 18; else if (verify.status !== 'unverified') s += 4;
...
async function runDeepDossier(rawInput) {
  ...
  const rows = await Promise.all(people.map(async (p) => {
    // Contact verification + the deeper "beyond Apollo" layer (Companies House +
    // recent news) run together per person, each failing soft on its own.
    // In mock mode use the synthetic CH/news baked into the row (fast, illustrative);
    // for real data hit Companies House + the news feed live.
    const [verify, ch, news] = await Promise.all([
