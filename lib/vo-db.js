...
async function ensure() {
  ...
  try {
    ...
    await sql`ALTER TABLE vo_campaigns ADD COLUMN IF NOT EXISTS schedule_days JSONB, ADD COLUMN IF NOT EXISTS timezone TEXT NOT NULL DEFAULT 'Europe/London', ADD COLUMN IF NOT EXISTS keywords_translated JSONB, ADD COLUMN IF NOT EXISTS next_run_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS last_run_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS auto_keywords JSONB`;
...
async function setAutoKeywords(id, list) { await ensure(); await sql`UPDATE vo_campaigns SET auto_keywords = ${jsonb(list || [])}::jsonb, updated_at = now() WHERE id = ${Number(id)}`; }
async function markTopup(id, note) { await ensure(); await sql`UPDATE vo_campaigns SET automation = COALESCE(automation, '{}'::jsonb) || ${jsonb({ last_topup_at: new Date().toISOString(), last_topup_note: note || '' })}::jsonb, updated_at = now() WHERE id = ${Number(id)}`; }
