...
async function ensure() {
  ...
  try {
    ...
    await sql`ALTER TABLE vo_prospects ADD COLUMN IF NOT EXISTS products JSONB, ADD COLUMN IF NOT EXISTS products_source TEXT, ADD COLUMN IF NOT EXISTS apollo_org_id TEXT, ADD COLUMN IF NOT EXISTS ad_analysis JSONB, ADD COLUMN IF NOT EXISTS last_checked_at TIMESTAMPTZ,
      ...
      ADD COLUMN IF NOT EXISTS email_sent_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS email_opened_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS meta_page_id TEXT, ADD COLUMN IF NOT EXISTS reply_sentiment TEXT, ADD COLUMN IF NOT EXISTS reply_summary TEXT`;
...
async function listCampaigns(account) {
  ...
  try {
    ...
    const { rows } = await sql`SELECT c.*,
        ...
        (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.campaign_id = c.id AND p.linkedin_connection_state = 'Applied') AS requested,
