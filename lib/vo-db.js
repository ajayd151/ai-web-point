...
async function ensure() {
  ...
  try {
    ...
    await sql`ALTER TABLE vo_runs ADD COLUMN IF NOT EXISTS state JSONB`;
    await sql`ALTER TABLE vo_prospects ADD COLUMN IF NOT EXISTS products JSONB, ADD COLUMN IF NOT EXISTS products_source TEXT, ADD COLUMN IF NOT EXISTS apollo_org_id TEXT, ADD COLUMN IF NOT EXISTS ad_analysis JSONB, ADD COLUMN IF NOT EXISTS last_checked_at TIMESTAMPTZ,
      ...
      ADD COLUMN IF NOT EXISTS email_sent_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS email_opened_at TIMESTAMPTZ`;
    ...
    await sql`INSERT INTO vo_config (key, value) VALUES ('exclusions', ${JSON.stringify(DEFAULT_EXCLUSIONS)}::jsonb) ON CONFLICT (key) DO NOTHING`;
    await sql`INSERT INTO vo_config (key, value) VALUES ('linkedin', ${JSON.stringify({ daily_requests: 20, daily_messages: 40, weekly_requests: 100, max_priority: 3, paused: false, paused_reason: '' })}::jsonb) ON CONFLICT (key) DO NOTHING`;
...
const EXTRA_COLS = ['ad_samples', 'products', 'products_source', 'apollo_org_id', 'ad_analysis'];
...
async function insertProspect(account, campaign, runId, p, s, msgs) {
  ...
  for (const f of EXTRA_COLS) { cols.push(f); vals.push(['ad_samples', 'products', 'ad_analysis'].includes(f) ? (p[f] == null ? null : JSON.stringify(p[f])) : (p[f] == null ? null : String(p[f]))); }
  const derived = { score_a: s.score_a, score_b: s.score_b, score_c: s.score_c, score_d: s.score_d, score_total: s.score_total, tier: s.tier, priority: s.priority, priority_number: s.priority_number, score_breakdown: JSON.stringify(s.breakdown), score_version: s.score_version,
  ...
  const JSONB = new Set(['score_breakdown', 'ad_samples', 'products', 'ad_analysis']);
...
async function saveRunState(run, state, status) {
  ...
  await sql`UPDATE vo_runs SET state = ${JSON.stringify(slim)}::jsonb, status = ${st}, counts = ${JSON.stringify(state.counts || {})}::jsonb, errors = ${JSON.stringify((state.errors || []).slice(0, 50))}::jsonb, actual_cost = ${Number(state.cost) || 0}, finished_at = COALESCE(${finished}, finished_at) WHERE id = ${Number(run.id)}`;
...
async function runningRuns(account) { await ensure(); const { rows } = await sql`SELECT * FROM vo_runs WHERE account = ${account} AND status IN ('Running','Queued') ORDER BY started_at ASC LIMIT 5`; return rows; }
async function stopRun(account, id, why) { await ensure(); await sql`UPDATE vo_runs SET status = 'Stopped (cap)', finished_at = now(), errors = COALESCE(errors, '[]'::jsonb) || ${JSON.stringify([why || 'stopped by user'])}::jsonb WHERE account = ${account} AND id = ${Number(id)} AND status IN ('Running','Queued')`; }
...
async function savePreset(account, name, keywords, translations) {
  ...
  const kw = JSON.stringify((keywords || []).map(String).filter(Boolean)); const tr = JSON.stringify(translations || {});
...
async function prospectsDueRecheck(account, limit) {
  await ensure();
  const { rows } = await sql`SELECT p.id, p.brand, p.domain, p.country, p.campaign_id, c.recheck_days FROM vo_prospects p JOIN vo_campaigns c ON c.id = p.campaign_id
...
async function deleteRunProspects(account, runId) { await ensure(); await sql`DELETE FROM vo_outreach_events WHERE account = ${account} AND prospect_id IN (SELECT id FROM vo_prospects WHERE account = ${account} AND run_id = ${Number(runId)})`; const r = await sql`DELETE FROM vo_prospects WHERE account = ${account} AND run_id = ${Number(runId)}`; return r.rowCount || 0; }
async function setProducts(id, products, source) { await ensure(); await sql`UPDATE vo_prospects SET products = ${JSON.stringify(products || [])}::jsonb, products_source = ${source || 'shopify'}, updated_at = now() WHERE id = ${Number(id)}`; }
...
async function linkedinQueue(account, maxPriority, limit) {
  ...
  const { rows } = await sql`SELECT p.* FROM vo_prospects p JOIN vo_campaigns c ON c.id = p.campaign_id
    WHERE p.account = ${account} AND c.status = 'Active' AND COALESCE((c.automation->>'auto_connect')::boolean, false) = true
      ...
      AND p.linkedin_connection_state IS NULL AND p.outreach_stage = 'Not contacted' AND COALESCE(p.dm_linkedin,'') <> '' AND COALESCE(p.tier,'') <> 'Disqualified'
...
async function recordReply(account, actor, id, text, atIso) {
  await ensure();
  const p = await getProspect(account, id); if (!p) return null;
  await sql`UPDATE vo_prospects SET last_reply_text = ${String(text || '').slice(0, 4000)}, last_reply_at = ${atIso || new Date().toISOString()}, updated_at = now() WHERE id = ${Number(id)}`;
  await cancelFollowups(id);
  if (stageAllowed(p.outreach_stage, 'Replied')) return setStage(account, actor, id, 'Replied', { channel: 'LinkedIn', note: 'Reply: ' + String(text || '').slice(0, 300) });
  await addEvent(account, actor, p, { channel: 'LinkedIn', step: 'Reply', response: String(text || '').slice(0, 1000) });
  return getProspect(account, id);
}
...
async function rescoreAll(account) {
  ...
  for (const p of rows) {
    const s = score(p, cfg);
    await sql`UPDATE vo_prospects SET score_a = ${s.score_a}, score_b = ${s.score_b}, score_c = ${s.score_c}, score_d = ${s.score_d}, score_total = ${s.score_total}, tier = ${s.tier}, priority = ${s.priority}, priority_number = ${s.priority_number}, score_breakdown = ${JSON.stringify(s.breakdown)}::jsonb, score_version = ${s.score_version}, updated_at = now() WHERE id = ${p.id}`;
    n++;
  }
  return n;
}
...
module.exports = { ENUM, STAGE_NEXT, stageAllowed, connAllowed, ensure, getConfig, setConfig, scoringConfig, serviceProfile, listCampaigns, getCampaign, saveCampaign, duplicateCampaign, createRun, finishRun, listRuns, importRows, listProspects, getProspect, updateProspect, setVideoUrl, addEvent, setStage, setConnectionState, setOutcome, PROSPECT_INPUTS,
  globalExclusions, linkedinSettings, existingDomains, insertProspect, getRun, saveRunState, runStore, runningRuns, stopRun, allAccounts, touchCampaignRun, scheduleDue, scheduledCampaigns, finishCampaign, listPresets, savePreset, deletePreset, prospectsDueRecheck, deleteProspect, deleteRunProspects, setProducts, markChecked, markEmail, prospectById, scheduleFollowups, dueFollowups, completeFollowup, cancelFollowups, linkedinQueue, countLinkedinSince, setLinkedinIds, appliedProspects, prospectByProviderId, usedOtherChannel, recordReply, readyToSend, results, validateScoring, scoringImpact, saveScoring, resetScoring, rescoreAll };
