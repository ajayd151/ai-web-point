...
async function reportData(account) {
  ...
  const daily = (await sql`SELECT d::date AS day,
    ...
    FROM generate_series((CURRENT_DATE - interval '13 days')::date, CURRENT_DATE, interval '1 day') AS d ORDER BY d`).rows.map((r) => ({ day: String(r.day).slice(0, 10), requests: r.requests, accepted: r.accepted, videos: r.videos, replies: r.replies, found: r.found }));
