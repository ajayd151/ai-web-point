...
async function reportData(account) {
  ...
  return { day, waiting, replied_open, events, note_test: await noteVariantStats(account), skipped: await skippedLinkedin(account), queue: (await linkedinQueue(account, li.max_priority || 4, 200)).length, followups_due: followups.length, campaigns: await listCampaigns(account), linkedin: li, today_requests: await countLinkedinSince(account, ['Request sent'], sod.toISOString()), week_requests: await countLinkedinSince(account, ['Request sent'], sow.toISOString()) };
}
