...
async function ensure() {
  ...
  try {
    ...
    await sql`ALTER TABLE vo_prospects ADD COLUMN IF NOT EXISTS products JSONB, ADD COLUMN IF NOT EXISTS products_source TEXT, ADD COLUMN IF NOT EXISTS apollo_org_id TEXT, ADD COLUMN IF NOT EXISTS ad_analysis JSONB, ADD COLUMN IF NOT EXISTS last_checked_at TIMESTAMPTZ,
      ...
      ADD COLUMN IF NOT EXISTS email_sent_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS email_opened_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS meta_page_id TEXT, ADD COLUMN IF NOT EXISTS reply_sentiment TEXT, ADD COLUMN IF NOT EXISTS reply_summary TEXT, ADD COLUMN IF NOT EXISTS product_candidates JSONB, ADD COLUMN IF NOT EXISTS linkedin_skip_reason TEXT, ADD COLUMN IF NOT EXISTS linkedin_note_variant TEXT, ADD COLUMN IF NOT EXISTS product_photo_url TEXT, ADD COLUMN IF NOT EXISTS linkedin_status_checked_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS person_search_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS person_search_result TEXT, ADD COLUMN IF NOT EXISTS alt_search_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS alt_search_result TEXT, ADD COLUMN IF NOT EXISTS prev_dm_name TEXT, ADD COLUMN IF NOT EXISTS prev_dm_title TEXT, ADD COLUMN IF NOT EXISTS prev_dm_linkedin TEXT, ADD COLUMN IF NOT EXISTS warmup_variant TEXT, ADD COLUMN IF NOT EXISTS warmup_liked_at TIMESTAMPTZ`;
    await sql`ALTER TABLE vo_campaigns ADD COLUMN IF NOT EXISTS schedule_days JSONB, ADD COLUMN IF NOT EXISTS timezone TEXT NOT NULL DEFAULT 'Europe/London', ADD COLUMN IF NOT EXISTS keywords_translated JSONB, ADD COLUMN IF NOT EXISTS next_run_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS last_run_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS auto_keywords JSONB, ADD COLUMN IF NOT EXISTS topup_keywords JSONB`;
...
async function updateProspect(account, actor, id, fields) {
  ...
  if (d.dm_linkedin !== undefined) { cols.push('linkedin_skip_reason'); vals.push(null); }
...
async function lastRunOfKind(account, campaignId, kind) { await ensure(); const { rows } = await sql`SELECT id, status, counts, started_at FROM vo_runs WHERE account = ${account} AND campaign_id = ${Number(campaignId)} AND kind = ${kind} ORDER BY started_at DESC LIMIT 1`; return rows[0] || null; }
// Auto top-up bookkeeping: extra shopper keywords the worker added, and when it last topped the queue up.
async function setAutoKeywords(id, list, fresh) { await ensure(); await sql`UPDATE vo_campaigns SET auto_keywords = ${jsonb(list || [])}::jsonb, topup_keywords = ${jsonb(fresh || [])}::jsonb, updated_at = now() WHERE id = ${Number(id)}`; }
async function markTopup(id, note, extra) { await ensure(); await sql`UPDATE vo_campaigns SET automation = COALESCE(automation, '{}'::jsonb) || ${jsonb(Object.assign({ last_topup_at: new Date().toISOString(), last_topup_note: note || '' }, extra || {}))}::jsonb, updated_at = now() WHERE id = ${Number(id)}`; }
...
async function scheduledCampaigns(account, opts) {
  await ensure();
  const { rows } = (opts && opts.any)
    ? await sql`SELECT c.*, (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.campaign_id = c.id AND COALESCE(p.tier,'') NOT IN ('Disqualified')) AS found FROM vo_campaigns c WHERE c.account = ${account} AND c.status = 'Active'`
    : await sql`SELECT c.*, (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.campaign_id = c.id AND COALESCE(p.tier,'') NOT IN ('Disqualified')) AS found FROM vo_campaigns c WHERE c.account = ${account} AND c.status = 'Active' AND c.schedule <> 'One-off'`;
  return rows;
}
...
// Follow-up 3: one last message 30 days after Follow-up 2, with a new reason (the season), never more than one.
async function scheduleFollowup3(account, actor, p, channel) {
  await ensure();
  const has = await sql`SELECT 1 FROM vo_outreach_events WHERE prospect_id = ${Number(p.id)} AND next_action = 'Follow-up 3' LIMIT 1`;
  if (has.rows.length) return false;
  const d3 = new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10);
  await addEvent(account, actor, p, { channel: channel, step: 'Follow-up 3 due', next_action: 'Follow-up 3', next_action_date: d3, detail: 'Draft ready, due ' + d3 });
  return true;
}
async function scheduleFollowups(account, actor, p, channel) {
  ...
  // spread over two weeks, so someone away for a week does not come back to a pile of messages (24 Sep 2026)
  const d1 = new Date(Date.now() + 4 * 86400000).toISOString().slice(0, 10); const d2 = new Date(Date.now() + 12 * 86400000).toISOString().slice(0, 10);
...
async function sentMessages(account, limit) {
  await ensure();
  const { rows } = await sql`SELECT e.at, e.step, e.channel, e.detail, p.id, p.brand, p.dm_name, p.outreach_stage, p.last_reply_at, p.reply_sentiment, p.reply_summary
    FROM vo_outreach_events e JOIN vo_prospects p ON p.id = e.prospect_id
    WHERE e.account = ${account} AND e.step IN ('Msg 1', 'Follow-up 1', 'Follow-up 2', 'Follow-up 1 sent', 'Follow-up 2 sent', 'Follow-up 3 sent', 'LinkedIn message', 'Email sent') AND COALESCE(p.source,'') <> 'demo'
    ORDER BY e.at DESC LIMIT ${Number(limit) || 50}`;
  return rows;
}
...
// Shortlisted brands with no personal LinkedIn link that LinkedIn search has not tried yet, best first.
async function needPerson(account, maxPriority, limit) {
  await ensure();
  const { rows } = await sql`SELECT p.* FROM vo_prospects p JOIN vo_campaigns c ON c.id = p.campaign_id
    WHERE p.account = ${account} AND c.status = 'Active' AND COALESCE(p.source,'') <> 'demo' AND COALESCE(p.tier,'') <> 'Disqualified'
      AND p.priority_number IS NOT NULL AND p.priority_number <= ${Number(maxPriority) || 4} AND p.outreach_stage = 'Not contacted' AND p.linkedin_connection_state IS NULL
      AND COALESCE(p.dm_linkedin,'') !~* '/in/' AND (p.person_search_at IS NULL OR (COALESCE(p.person_search_result,'') NOT LIKE 'v3%' AND COALESCE(p.person_search_result,'') <> 'found'))
    ORDER BY p.priority_number ASC, p.score_total DESC LIMIT ${Number(limit) || 1}`;
  return rows;
}
// Brands whose contact was found inactive on LinkedIn and no switch has been tried yet.
async function needAltPerson(account, maxPriority, limit) {
  await ensure();
  const { rows } = await sql`SELECT p.* FROM vo_prospects p JOIN vo_campaigns c ON c.id = p.campaign_id
    WHERE p.account = ${account} AND c.status = 'Active' AND COALESCE(p.source,'') <> 'demo' AND COALESCE(p.tier,'') <> 'Disqualified'
      AND p.priority_number IS NOT NULL AND p.priority_number <= ${Number(maxPriority) || 4} AND p.outreach_stage = 'Not contacted' AND p.linkedin_connection_state IS NULL
      AND p.dm_active_90d = 'N' AND p.alt_search_at IS NULL
    ORDER BY p.priority_number ASC, p.score_total DESC LIMIT ${Number(limit) || 1}`;
  return rows;
}
async function markAltSearch(id, result) { await ensure(); await sql`UPDATE vo_prospects SET alt_search_at = now(), alt_search_result = ${String(result || '').slice(0, 200)} WHERE id = ${Number(id)}`; }
async function keepPreviousPerson(id, name, title, url) { await ensure(); await sql`UPDATE vo_prospects SET prev_dm_name = ${name || null}, prev_dm_title = ${title || null}, prev_dm_linkedin = ${url || null} WHERE id = ${Number(id)}`; }
async function clearProviderId(id) { await ensure(); await sql`UPDATE vo_prospects SET linkedin_provider_id = NULL WHERE id = ${Number(id)}`; }
async function markPersonSearch(id, result) { await ensure(); await sql`UPDATE vo_prospects SET person_search_at = now(), person_search_result = ${String(result || '').slice(0, 200)} WHERE id = ${Number(id)}`; }
async function lastStepAt(prospectId, step) { await ensure(); const { rows } = await sql`SELECT MAX(at) AS at FROM vo_outreach_events WHERE prospect_id = ${Number(prospectId)} AND step = ${step}`; return rows[0] && rows[0].at ? new Date(rows[0].at) : null; }
...
// Contacted brands missing their next follow-up (sent by hand, or before a follow-up existed), so none is left without one.
// A follow-up that was skipped counts as handled and is never put back.
async function followupGaps(account) {
  await ensure();
  const { rows } = await sql`SELECT p.id, p.account, p.campaign_id, p.brand, p.dm_name, p.outreach_stage FROM vo_prospects p
    WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND p.outreach_stage IN ('Msg 1', 'Follow-up 1', 'Follow-up 2')
      AND NOT EXISTS (SELECT 1 FROM vo_outreach_events e WHERE e.prospect_id = p.id AND e.next_action_date IS NOT NULL AND e.done IS NOT TRUE)
      AND NOT EXISTS (SELECT 1 FROM vo_outreach_events e WHERE e.prospect_id = p.id AND e.next_action = (CASE p.outreach_stage WHEN 'Msg 1' THEN 'Follow-up 1' WHEN 'Follow-up 1' THEN 'Follow-up 2' ELSE 'Follow-up 3' END))
    ORDER BY p.id LIMIT 50`;
  return rows;
}
async function scheduleOne(account, actor, p, which, days, channel) {
  const d = new Date(Date.now() + days * 86400000).toISOString().slice(0, 10);
  await addEvent(account, actor, p, { channel: channel || 'LinkedIn', step: which + ' due', next_action: which, next_action_date: d, detail: 'Draft ready, due ' + d });
  return d;
}
// Follow-ups booked but not due yet, soonest first, for the Scheduled follow-ups list.
async function upcomingFollowups(account) {
  await ensure();
  const { rows } = await sql`SELECT e.id AS event_id, e.next_action, e.next_action_date, e.channel, p.id, p.brand, p.dm_name, p.country, p.outreach_stage
    FROM vo_outreach_events e JOIN vo_prospects p ON p.id = e.prospect_id
    WHERE e.account = ${account} AND e.next_action_date IS NOT NULL AND e.done IS NOT TRUE AND e.next_action_date > CURRENT_DATE AND COALESCE(p.source,'') <> 'demo'
      AND p.outreach_stage NOT IN ('Replied','Call booked','Pilot','Client','Dead')
    ORDER BY e.next_action_date ASC LIMIT 100`;
  return rows;
}
...
async function linkedinQueue(account, maxPriority, limit) {
  ...
  const { rows } = await sql`SELECT p.* FROM vo_prospects p JOIN vo_campaigns c ON c.id = p.campaign_id
    WHERE p.account = ${account} AND c.status = 'Active' AND COALESCE((c.automation->>'auto_connect')::boolean, false) = true
      ...
      AND p.linkedin_connection_state IS NULL AND p.outreach_stage = 'Not contacted' AND COALESCE(p.dm_linkedin,'') <> '' AND p.linkedin_skip_reason IS NULL AND COALESCE(p.tier,'') <> 'Disqualified' AND COALESCE(p.source,'') <> 'demo'
      AND COALESCE(p.dm_active_90d,'') <> 'N'
      AND NOT (COALESCE(p.warmup_variant,'') = 'warm' AND p.warmup_liked_at IS NOT NULL AND p.warmup_liked_at > now() - interval '20 hours')
    ORDER BY (p.dm_active_90d = 'Y') DESC, p.priority_number ASC, p.score_total DESC LIMIT ${Number(limit) || 1}`;
  return rows;
}
// The queue skipped this brand (bad LinkedIn link, provider refused the profile). Cleared when the link is edited or outreach is reset.
// Connection note 50:50 test: which note went with the request, and the outcome per note.
async function setNoteVariant(id, v) { await ensure(); await sql`UPDATE vo_prospects SET linkedin_note_variant = ${v || null}, updated_at = now() WHERE id = ${Number(id)}`; }
async function setWarmup(id, variant, likedAt) { await ensure(); await sql`UPDATE vo_prospects SET warmup_variant = ${variant}, warmup_liked_at = ${likedAt || null} WHERE id = ${Number(id)}`; }
async function warmupStats(account) {
  await ensure();
  const { rows } = await sql`SELECT warmup_variant AS variant, COUNT(*) FILTER (WHERE linkedin_request_sent_at IS NOT NULL)::int AS sent,
      COUNT(*) FILTER (WHERE linkedin_connected_at IS NOT NULL AND linkedin_request_sent_at IS NOT NULL)::int AS accepted,
      COUNT(*) FILTER (WHERE warmup_variant = 'warm' AND warmup_liked_at IS NOT NULL)::int AS liked,
      COUNT(*)::int AS assigned
    FROM vo_prospects WHERE account = ${account} AND warmup_variant IS NOT NULL AND COALESCE(source,'') <> 'demo' GROUP BY 1 ORDER BY 1`;
  return rows;
}
async function noteVariantStats(account) {
  await ensure();
  const { rows } = await sql`SELECT COALESCE(linkedin_note_variant, 'offer') AS variant, COUNT(*)::int AS sent,
      COUNT(*) FILTER (WHERE linkedin_connected_at IS NOT NULL)::int AS accepted,
      COUNT(*) FILTER (WHERE linkedin_connection_state = 'Applied' AND linkedin_request_sent_at > now() - interval '7 days')::int AS open,
      COUNT(*) FILTER (WHERE last_reply_at IS NOT NULL)::int AS replied,
      COUNT(*) FILTER (WHERE reply_sentiment = 'Positive')::int AS positive
    FROM vo_prospects WHERE account = ${account} AND linkedin_request_sent_at IS NOT NULL AND COALESCE(source,'') <> 'demo' GROUP BY 1 ORDER BY 1`;
  return rows;
}
async function setProductPhoto(account, actor, id, url, productName) {
  await ensure();
  const p = await getProspect(account, id); if (!p) throw new Error('Prospect not found');
  await sql`UPDATE vo_prospects SET product_photo_url = ${url || null}, updated_at = now() WHERE id = ${Number(id)}`;
  if (productName && !p.suggested_product_name) await updateProspect(account, actor, id, { suggested_product_name: productName, product_photo_check: 'Unverified', why_this_product: 'picked by hand with an uploaded photo' });
  await addEvent(account, actor, p, { step: 'note', detail: 'Product photo uploaded by hand' });
  return getProspect(account, id);
}
async function setLinkedinSkip(id, reason) { await ensure(); await sql`UPDATE vo_prospects SET linkedin_skip_reason = ${reason ? String(reason).slice(0, 300) : null}, updated_at = now() WHERE id = ${Number(id)}`; }
async function skippedLinkedin(account) { await ensure(); const { rows } = await sql`SELECT p.id, p.brand, p.dm_name, p.dm_linkedin, p.linkedin_skip_reason FROM vo_prospects p JOIN vo_campaigns c ON c.id = p.campaign_id WHERE p.account = ${account} AND c.status = 'Active' AND p.linkedin_skip_reason IS NOT NULL AND p.linkedin_connection_state IS NULL ORDER BY p.priority_number ASC NULLS LAST, p.score_total DESC LIMIT 50`; return rows; }
async function countLinkedinSince(account, steps, sinceIso) {
  await ensure();
  const { rows } = await q("SELECT COUNT(*)::int AS n FROM vo_outreach_events e JOIN vo_prospects p ON p.id = e.prospect_id WHERE e.account = $1 AND e.channel = $2 AND e.step = ANY($3) AND e.at >= $4 AND COALESCE(p.source,'') <> 'demo'", [account, 'LinkedIn', steps, sinceIso]);
...
// Queue candidates whose LinkedIn activity is still unknown (the activity gate checks these before any request goes out)
async function unknownActivity(account, maxPriority, limit) {
  await ensure();
  const { rows } = await sql`SELECT p.* FROM vo_prospects p JOIN vo_campaigns c ON c.id = p.campaign_id
    WHERE p.account = ${account} AND c.status = 'Active' AND COALESCE((c.automation->>'auto_connect')::boolean, false) = true
      AND p.priority_number IS NOT NULL AND p.priority_number <= LEAST(${Number(maxPriority) || 3}, COALESCE((c.automation->>'max_priority')::int, 2))
      AND p.linkedin_connection_state IS NULL AND p.outreach_stage = 'Not contacted' AND COALESCE(p.dm_linkedin,'') <> '' AND p.linkedin_skip_reason IS NULL AND COALESCE(p.tier,'') <> 'Disqualified' AND COALESCE(p.source,'') <> 'demo'
      AND COALESCE(p.dm_active_90d,'') NOT IN ('Y','N')
    ORDER BY p.priority_number ASC, p.score_total DESC LIMIT ${Number(limit) || 10}`;
  return rows;
}
async function openForStatusCheck(account, limit) { await ensure(); const { rows } = await sql`SELECT * FROM vo_prospects WHERE account = ${account} AND linkedin_connection_state IN ('Applied','Pending') AND linkedin_provider_id IS NOT NULL AND COALESCE(source,'') <> 'demo' ORDER BY linkedin_status_checked_at ASC NULLS FIRST, linkedin_request_sent_at ASC LIMIT ${Number(limit) || 30}`; return rows; }
async function markStatusChecked(id) { await ensure(); await sql`UPDATE vo_prospects SET linkedin_status_checked_at = now() WHERE id = ${Number(id)}`; }
...
async function readyToSend(account) { await ensure(); const { rows } = await sql`SELECT p.*, c.name AS campaign_name FROM vo_prospects p JOIN vo_campaigns c ON c.id = p.campaign_id WHERE p.account = ${account} AND p.linkedin_connection_state = 'Connected' AND p.outreach_stage IN ('Accepted','Request sent','Not contacted') ORDER BY (COALESCE(p.source,'') = 'demo') ASC, p.linkedin_connected_at DESC NULLS LAST, p.priority_number ASC LIMIT 200`; return rows; }
...
async function createDemoReady(account, actor) {
  ...
  const KL = [{"handle": "fathers-day-tradesman-the-weekend-warrior-kit", "title": "The Ultimate Tradesman Toolkit", "price": 95.64, "tags": ["bundles", "father's day bundle", "Tool Box Bundles"], "created_at": null, "images": [{"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/hero_new_59e0a9ca-4f97-4bb8-893b-d865b53d7d6e.png?v=1780057324", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/img_6_0349b970-1657-4b8b-842e-47fedc87f02f.png?v=1779458329", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/img_2_1095feaf-2e2b-44d8-83f2-9034bbb2be07.png?v=1779457910", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/img_4_85f88a5e-2e05-4573-beb8-2980da0f9b6f.png?v=1779457910", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/image_2631657e-f254-44db-84b9-2db092ba2a34.png?v=1779361332", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/img_3_642b9174-5d51-462a-9c53-828ce0d435a4.png?v=1779457910", "alt": ""}], "url": "https://klevaro.co.uk/products/fathers-day-tradesman-the-weekend-warrior-kit"}, {"handle": "fathers-day-starter-the-first-fix-kit", "title": "The Ultimate DIY Starter Bundle", "price": 49.99, "tags": ["Tool box bundles"], "created_at": null, "images": [{"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/updatedimage1.png?v=1781513840", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/updateimg2.png?v=1781513839", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/updatedimage3.png?v=1781513840", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/updatedimage4.png?v=1781513840", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/image_52228c8d-8fa8-43d1-ad73-0c44ced51617.png?v=1781513840", "alt": ""}], "url": "https://klevaro.co.uk/products/fathers-day-starter-the-first-fix-kit"}, {"handle": "fathers-day-pro-the-master-craftsman-kit", "title": "The Ultimate Professional Toolkit", "price": 159.4, "tags": ["bundles", "father's day gift", "Tool Box Bundles"], "created_at": null, "images": [{"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/hero_new_061949f2-d095-4786-9f7a-c7311273eaed.png?v=1780057621", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/im3.png?v=1779915347", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/img_2_c93c9787-7706-4ff5-bb16-67f875be7e6a.png?v=1779915347", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/img_6_575f86a0-4732-4154-9393-cba3d83ac54d.png?v=1779915348", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/img_5_b2e13791-d489-451b-8511-b6de6e12cc01.png?v=1779915347", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/img_4_7ab8f790-5c0e-4b9d-9279-ea5207eab4a7.png?v=1779915347", "alt": ""}, {"src": "https://cdn.shopify.com/s/files/1/0905/1055/7511/files/image_aae49ce8-b7ed-4607-97ac-ac15e9579363.png?v=1779915362", "alt": ""}], "url": "https://klevaro.co.uk/products/fathers-day-pro-the-master-craftsman-kit"}];
  const p = { brand: 'Klevaro (example)', website: 'klevaro.co.uk', domain: 'example.klevaro.co.uk', country: 'UK', category: 'DIY tools and gifts', source: 'demo', active_meta_ads: 48, video_ads: 30, new_ads_30d: 9, other_paid_channels: 1, creative_style: 'Video-led', skus: 30, employees: 18, growth_signals: 2, pays_for_creative: true, video_sourcing: 'UGC creators', creative_gap: 4, trigger_event: true, trigger_note: 'Launched 9 new ads this month', dm_name: 'Jamie Example', dm_title: 'Founder', dm_linkedin: 'linkedin.com/in/jamie-example', dm_active_90d: 'Y', gatekeeper: false, second_contact_has_email: false, observation: 'your Ultimate Tradesman Toolkit video ads on Meta', suggested_product_url: 'https://klevaro.co.uk/products/fathers-day-tradesman-the-weekend-warrior-kit', suggested_product_name: 'The Ultimate Tradesman Toolkit', product_photo_check: 'Pass (6)', why_this_product: 'named most in their current ads, 6 real photos', notes: 'Example record so you can see how Ready to send works. Klevaro is a real UK store used for the demo; the person and the numbers are made up.', brand_instagram: '', dm_email: '', disqualified_reason: '', products: KL, products_source: 'shopify' };
  ...
  p.product_candidates = [{ url: KL[0].url, name: KL[0].title, photos: 6, why: 'named most in their current ads' }, { url: KL[1].url, name: KL[1].title, photos: 5, why: 'featured on their homepage' }, { url: KL[2].url, name: KL[2].title, photos: 7, why: 'matches what the campaign is about' }];
...
// Rebuild the three messages for every prospect of the account (after a profile or template change). Keeps the video link.
async function regenerateMessages(account, actor) {
  await ensure();
  const { rows } = await sql`SELECT * FROM vo_prospects WHERE account = ${account} AND outreach_stage IN ('Not contacted', 'Request sent', 'Accepted')`;
  let n = 0;
  for (const p of rows) {
    const campaign = await getCampaign(account, p.campaign_id);
    const { msgs } = await computeDerived(p, campaign); if (!msgs) continue;
    await sql`UPDATE vo_prospects SET connection_note = ${msgs.connection_note}, message_a = ${msgs.message_a}, message_b = ${msgs.message_b}, updated_at = now() WHERE id = ${p.id}`;
    n++;
  }
  try { await logActivity(actor, account, 'vo_regenerate', n + ' prospect messages rebuilt from the current profile'); } catch (e) {}
  return n;
}

// ---- daily report data ----
// Funnel between two dates: each stage is counted by when it happened (found, requested, accepted, video sent, replied), so a
// range shows what that period produced rather than the state of the brands found in it.
async function funnelData(account, fromIso, toIso) {
  await ensure();
  const from = fromIso ? new Date(fromIso) : new Date('2026-09-01T00:00:00Z'); const to = toIso ? new Date(new Date(toIso).getTime() + 86400000) : new Date(Date.now() + 86400000);
  const f = from.toISOString(); const t = to.toISOString();
  const r = (await sql`SELECT
      (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND p.created_at >= ${f} AND p.created_at < ${t}) AS brands,
      (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND p.created_at >= ${f} AND p.created_at < ${t} AND COALESCE(p.tier,'') <> 'Disqualified' AND p.priority_number <= 4) AS shortlisted,
      (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND p.created_at >= ${f} AND p.created_at < ${t} AND COALESCE(p.tier,'') <> 'Disqualified' AND p.priority_number <= 4 AND p.dm_linkedin ~* '/in/') AS with_linkedin,
      (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND p.created_at >= ${f} AND p.created_at < ${t} AND COALESCE(p.tier,'') <> 'Disqualified' AND p.priority_number <= 4 AND p.dm_linkedin ~* '/in/' AND p.dm_active_90d = 'Y') AS active,
      (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND p.linkedin_request_sent_at >= ${f} AND p.linkedin_request_sent_at < ${t}) AS requested,
      (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND p.linkedin_connected_at >= ${f} AND p.linkedin_connected_at < ${t}) AS accepted,
      (SELECT COUNT(DISTINCT e.prospect_id)::int FROM vo_outreach_events e JOIN vo_prospects p ON p.id = e.prospect_id WHERE e.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND e.step IN ('Msg 1','LinkedIn message','Email sent') AND e.at >= ${f} AND e.at < ${t}) AS videos,
      (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND p.last_reply_at >= ${f} AND p.last_reply_at < ${t}) AS replied,
      (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND p.last_reply_at >= ${f} AND p.last_reply_at < ${t} AND p.reply_sentiment = 'Positive') AS positive,
      (SELECT COUNT(DISTINCT e.prospect_id)::int FROM vo_outreach_events e JOIN vo_prospects p ON p.id = e.prospect_id WHERE e.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND e.step IN ('Call booked','Pilot','Client') AND e.at >= ${f} AND e.at < ${t}) AS calls,
      (SELECT MIN(created_at) FROM vo_prospects WHERE account = ${account} AND COALESCE(source,'') <> 'demo') AS first_ever`).rows[0] || {};
  return Object.assign(r, { from: f.slice(0, 10), to: new Date(to.getTime() - 86400000).toISOString().slice(0, 10) });
}
async function reportData(account) {
  ...
  const daily = (await sql`SELECT to_char(d::date, 'YYYY-MM-DD') AS day,
      (SELECT COUNT(*)::int FROM vo_outreach_events e JOIN vo_prospects p ON p.id = e.prospect_id WHERE e.account = ${account} AND e.step = 'Request sent' AND COALESCE(p.source,'') <> 'demo' AND e.at::date = d::date) AS requests,
      (SELECT COUNT(*)::int FROM vo_outreach_events e JOIN vo_prospects p ON p.id = e.prospect_id WHERE e.account = ${account} AND e.step = 'Connection Connected' AND COALESCE(p.source,'') <> 'demo' AND e.at::date = d::date) AS accepted,
      (SELECT COUNT(*)::int FROM vo_outreach_events e JOIN vo_prospects p ON p.id = e.prospect_id WHERE e.account = ${account} AND e.step IN ('Msg 1','LinkedIn message','Email sent') AND COALESCE(p.source,'') <> 'demo' AND e.at::date = d::date) AS videos,
      (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND p.last_reply_at::date = d::date) AS replies,
      (SELECT COUNT(*)::int FROM vo_prospects p WHERE p.account = ${account} AND COALESCE(p.source,'') <> 'demo' AND COALESCE(p.tier,'') <> 'Disqualified' AND p.created_at::date = d::date) AS found
    FROM generate_series((CURRENT_DATE - interval '13 days')::date, CURRENT_DATE, interval '1 day') AS d ORDER BY d`).rows.map((r) => ({ day: String(r.day), requests: r.requests, accepted: r.accepted, videos: r.videos, replies: r.replies, found: r.found }));
  const f = (await sql`SELECT
      COUNT(*)::int AS brands,
      COUNT(*) FILTER (WHERE COALESCE(tier,'') <> 'Disqualified' AND priority_number <= 4)::int AS shortlisted,
      COUNT(*) FILTER (WHERE COALESCE(tier,'') <> 'Disqualified' AND priority_number <= 4 AND dm_linkedin ~* '/in/')::int AS with_linkedin,
      COUNT(*) FILTER (WHERE COALESCE(tier,'') <> 'Disqualified' AND priority_number <= 4 AND dm_linkedin ~* '/in/' AND dm_active_90d = 'Y')::int AS active,
      COUNT(*) FILTER (WHERE linkedin_request_sent_at IS NOT NULL)::int AS requested,
      COUNT(*) FILTER (WHERE linkedin_connection_state = 'Connected')::int AS accepted,
      COUNT(*) FILTER (WHERE outreach_stage IN ('Msg 1','Follow-up 1','Follow-up 2','Replied','Call booked','Pilot','Client'))::int AS videos,
      COUNT(*) FILTER (WHERE last_reply_at IS NOT NULL)::int AS replied,
      COUNT(*) FILTER (WHERE reply_sentiment = 'Positive')::int AS positive,
      COUNT(*) FILTER (WHERE outreach_stage IN ('Call booked','Pilot','Client'))::int AS calls,
      MIN(created_at) AS since
    FROM vo_prospects WHERE account = ${account} AND COALESCE(source,'') <> 'demo'`).rows[0] || {};
  f.found_by_search = (await sql`SELECT COUNT(*)::int AS n FROM vo_prospects WHERE account = ${account} AND person_search_result = 'found'`).rows[0].n;
  f.switched = (await sql`SELECT COUNT(*)::int AS n FROM vo_prospects WHERE account = ${account} AND alt_search_result = 'switched'`).rows[0].n;
  const warmup = await warmupStats(account);
  return { day, waiting, replied_open, events, warmup: warmup, funnel: f, daily: daily, last_tick: await getConfig('last_tick', null), note_test: await noteVariantStats(account), skipped: await skippedLinkedin(account), queue: (await linkedinQueue(account, li.max_priority || 4, 200)).length, followups_due: followups.length, campaigns: await listCampaigns(account), linkedin: li, today_requests: await countLinkedinSince(account, ['Request sent'], sod.toISOString()), week_requests: await countLinkedinSince(account, ['Request sent'], sow.toISOString()) };
}
...
module.exports = { ENUM, STAGE_NEXT, stageAllowed, connAllowed, ensure, getConfig, setConfig, scoringConfig, serviceProfile, listCampaigns, getCampaign, saveCampaign, duplicateCampaign, createRun, finishRun, listRuns, importRows, listProspects, getProspect, updateProspect, setVideoUrl, addEvent, setStage, setConnectionState, setOutcome, resetOutreach, PROSPECT_INPUTS,
  globalExclusions, linkedinSettings, existingDomains, insertProspect, getRun, saveRunState, runStore, runningRuns, claimRun, stopRun, allAccounts, touchCampaignRun, scheduleDue, scheduledCampaigns, finishCampaign, listPresets, savePreset, deletePreset, prospectsDueRecheck, countRecountable, deleteProspect, deleteRunProspects, setProducts, markChecked, markEmail, prospectById, scheduleFollowups, dueFollowups, followupGaps, scheduleOne, upcomingFollowups, completeFollowup, cancelFollowups, sentMessages, lastStepAt, scheduleFollowup3, setWarmup, warmupStats, needPerson, markPersonSearch, needAltPerson, markAltSearch, keepPreviousPerson, clearProviderId, setProductPhoto, unknownActivity, openForStatusCheck, markStatusChecked, linkedinQueue, setLinkedinSkip, skippedLinkedin, setNoteVariant, noteVariantStats, setAutoKeywords, markTopup, lastRunOfKind, countLinkedinSince, setLinkedinIds, appliedProspects, prospectByProviderId, usedOtherChannel, recordReply, readyToSend, reportData, saveQuestion, listQuestions, markQuestion, faqExtra, addFaq, removeFaq, createDemoReady, removeDemo, regenerateMessages, funnelData, results, validateScoring, scoringImpact, saveScoring, resetScoring, rescoreAll };
