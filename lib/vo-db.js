...
// Postgres jsonb refuses lone surrogates (an emoji cut in half) and function jsonb(v) { return JSON.stringify(v == null ? null : v).replace(/[\ud800-\udbff](?![\udc00-\udfff])/g, '').replace(/(^|[^\ud800-\udbff])[\udc00-\udfff]/g, '$1').replace(/\\u0000/g, ''); }
...
async function insertProspect(account, campaign, runId, p, s, msgs) {
  ...
  for (const f of EXTRA_COLS) { cols.push(f); vals.push(['ad_samples', 'products', 'ad_analysis'].includes(f) ? (p[f] == null ? null : jsonb(p[f])) : (p[f] == null ? null : String(p[f]))); }
  const derived = { score_a: s.score_a, score_b: s.score_b, score_c: s.score_c, score_d: s.score_d, score_total: s.score_total, tier: s.tier, priority: s.priority, priority_number: s.priority_number, score_breakdown: jsonb(s.breakdown), score_version: s.score_version,
...
async function setProducts(id, products, source) { await ensure(); await sql`UPDATE vo_prospects SET products = ${jsonb(products || [])}::jsonb, products_source = ${source || 'shopify'}, updated_at = now() WHERE id = ${Number(id)}`; }
