...
async function saveRunState(run, state, status) {
  ...
  await sql`UPDATE vo_runs SET state = ${JSON.stringify(slim)}::jsonb, status = ${st}, counts = ${JSON.stringify(state.counts || {})}::jsonb, errors = ${JSON.stringify((state.errors || []).slice(0, 50))}::jsonb, actual_cost = ${Number(state.cost) || 0}, heartbeat = now(), finished_at = COALESCE(${finished}, finished_at) WHERE id = ${Number(run.id)}`;
