...
const { sentKeys, optoutSet, sentPhones } = require('./smsdb');
...
async function buildAudience(filters, opts) {
  ...
  const sentPh = await sentPhones();          // phones we have already texted (any campaign)
  const seenPhone = new Set();                 // phones queued within THIS build (dedupe same-number records)
  ...
  for (const c of Object.values(calls)) {
    ...
    if (st === 'dnd') { skipped.dnd = (skipped.dnd || 0) + 1; continue; } // never contact a Do-Not-Contact record
    ...
    // never text the same NUMBER twice: already texted before, or a duplicate record in this build
    if (notMessaged && sentPh.has(mob)) { skipped.alreadyMessaged++; continue; }
    if (seenPhone.has(mob)) { skipped.duplicateNumber = (skipped.duplicateNumber || 0) + 1; continue; }
    seenPhone.add(mob);
    matched++;
    // real Google numbers for the {reviews} / {rating} tags (record first, then enrichment)
    const revs = (c.reviews != null && c.reviews !== '') ? Number(c.reviews) : (ed.reviews != null ? Number(ed.reviews) : null);
    const rate = (c.rating != null && c.rating !== '') ? Number(c.rating) : (ed.rating != null ? Number(ed.rating) : null);
    if (out.length < max) out.push({ key: c.key, name: c.name, location: c.location || '', category: c.tag || c.category || '', phone: mob, reviews: revs, rating: rate });
  }
