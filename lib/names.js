...
// fluff adjectives + venue words people drop when they say the name casually ("Nana's Hair &
// Beauty Parlour" -> "Nana's Hair & Beauty"). Only dropped when 2+ real words remain, so a name
// that IS just its venue ("Serenity Spa", "Headmasters Salon") is left alone.
const FLUFF = new Set(('independent professional certified registered qualified experienced reliable trusted established genuine approved accredited insured dependable ' +
...
function humaniseBusinessName(name) {
  ...
  if (!raw) return raw;
  // collapse dotted acronyms: "P.M.K." -> "PMK", "A.B.C" -> "ABC"
  raw = raw.split(/\s+/).map((t) => (/^[A-Za-z](\.[A-Za-z])+\.?$/.test(t) ? t.replace(/\./g, '').toUpperCase() : t)).join(' ');
