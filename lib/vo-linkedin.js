...
const NOT_BUYER_RE = /\b(editor|freelance|self-employed|intern|student|assistant|ugc creator|content creator|designer)\b/i;
...
function pickFromSearch(items, brand, domain) {
  ...
  const cands = (items || []).filter((x) => /\/in\//.test(x.url || '') && worksThere(x.headline) && PERSON_TITLE_RE.test(x.headline) && !NOT_BUYER_RE.test(x.headline.split(/\s+(?:at|@|\|)\s+/i)[0]));
...
module.exports = { providerName, enabled, provider, dry, unipile, limits, inSendWindow, personProfile, activeSince, ACTIVE_DAYS, cleanBrand, pickFromSearch, companyMatches, PERSON_TITLE_RE, NOT_BUYER_RE, lastSearchDebug, HARD_DAILY_REQUESTS };
