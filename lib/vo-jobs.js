...
// SMS alert to the mobiles in Settings (vo_config 'alerts'). Default: Positive and Question replies only.
async function replyAlerts(account, p, cls, channel, base) {
  const a = await db.getConfig('alerts', {}) || {}; const mobiles = (a.mobiles || []).map((m) => String(m).trim()).filter(Boolean);
  const want = a.all_replies ? true : ['Positive', 'Question'].includes(cls.sentiment);
  const out = { sms: 0 };
  if (!mobiles.length || !want) return out;
  const text = (cls.sentiment || 'New') + ' reply for ShekiPro from ' + (p.dm_name || 'the contact') + ' at ' + p.brand + ' via ' + (channel || 'LinkedIn') + (cls.summary ? ': ' + cls.summary : '') + '. Next: open SitePounce > Video Outreach > ' + p.brand + ' and answer them. ' + (base || 'https://www.sitepounce.com');
  for (const m of mobiles) { try { const r = await sendSms(m, text.slice(0, 320), base); if (r && r.ok !== false) out.sms++; } catch (e) { /* SMS is best effort */ } }
  return out;
}
...
module.exports = { replyAlerts, startRun, stepRun, continueRuns, startScheduled, recheck, sendEmail, sendFollowup, autoFollowups, linkedinTick, linkedinSend, pauseLinkedin, tick };
