...
async function refreshProducts(account, actor, p, campaign) {
  ...
  let products = shop.products, source = 'shopify';
  if (!shop.is_shopify) {
    products = S.productsFromAds(p.ad_samples || [], p.domain); source = 'ads';
    if (!products.length) return { ok: false, error: 'No Shopify products feed at ' + p.domain + (shop.status ? ' (HTTP ' + shop.status + ')' : '') + ' and no product links in their ads. Type the product by hand.' };
  }
