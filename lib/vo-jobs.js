...
async function newLeadAlerts(account, p, base) {
  ...
  return smsAll(targets, (t) => (t.name ? t.name + ', ' : '') + 'you have a new lead for ShekiPro: ' + (p.dm_name || 'the contact') + ' at ' + p.brand + ' accepted on LinkedIn. Create an AI video for ' + (p.suggested_product_name || 'their hero product') + ', then send it via SitePounce > Video Outreach > Ready to send.', base);
}
...
async function testAlerts(account, base) {
  ...
  return smsAll(targets, (t) => (t.name ? t.name + ', ' : '') + 'this is a test from SitePounce Video Outreach. Real texts will read: "you have a new lead for ShekiPro: Dan McCormick at Create accepted on LinkedIn. Create an AI video for Creatine Gummies, then send it via SitePounce > Video Outreach > Ready to send."', base);
}
