...
async function newLeadAlerts(account, p, base) {
  ...
  return smsAll(targets, (t) => (t.name ? t.name + ', ' : '') + 'new ShekiPro connection: ' + (p.dm_name || 'the contact') + ' at ' + p.brand + ' accepted on LinkedIn. Create an AI video for ' + (p.suggested_product_name || 'their hero product') + ', then paste the link and send here: ' + link, base);
}
  ...
  try {
    ...
    for (const p of await db.appliedProspects(account)) {
      ...
      if (p.linkedin_provider_id && accepted.has(String(p.linkedin_provider_id))) {
        ...
        E.notifyOwner('New ShekiPro connection: ' + p.brand + ' accepted on LinkedIn', (p.dm_name || 'The contact') + ' at ' + p.brand + ' accepted the connection request.\n\nCreate the AI video for ' + (p.suggested_product_name || 'their hero product') + ', then paste the link and send it here: ' + (o.base || 'https://www.sitepounce.com') + '/#vo-ready-' + p.id);
        out.accepted++;
