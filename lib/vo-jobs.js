...
async function topUp(account, actor, now) {
  ...
  for (const c of await db.scheduledCampaigns(account, { any: true })) {
    ...
    if (auto.last_topup_at && t - new Date(auto.last_topup_at) < 20 * 3600000) continue;
    ...
    if (auto.expand_keywords !== false) {
      try {
        ...
        const sug = await S.suggestKeywords(c.industry || (c.keywords || [])[0], c.countries, c.language);
        // shopper intent only: Ajay prunes research terms (benefits, review, dosage, side effects, how to, vs) by hand, so never add them
        ...
        const fresh = (sug.keywords || []).filter((k) => !have.has(k) && !research.test(k)).slice(0, 6);
        if (fresh.length && (c.auto_keywords || []).length < 30) { const list = (c.auto_keywords || []).concat(fresh); await db.setAutoKeywords(c.id, list); c.auto_keywords = list; note += ', added keywords: ' + fresh.join(', '); }
      } catch (e) { note += ', keyword suggestion failed: ' + e.message; }
    }
    const run = await startRun(account, actor, c, 'top-up');
    await db.markTopup(c.id, note);
