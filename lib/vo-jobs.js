...
async function continueRuns(account, actor, budgetMs) {
  ...
  for (const run of await db.runningRuns(account)) {
...
async function recheck(account, actor, limit) {
  const out = [];
  for (const p of await db.prospectsDueRecheck(account, limit || 3)) {
    try {
      const ads = await S.apifyMetaAds({ keywords: [p.brand], country: p.country || 'US', limit: 100, timeoutSec: 40 });
      const g = S.groupAds(ads.rows).find((x) => (x.domain && x.domain === p.domain) || (x.page_name && x.page_name.toLowerCase() === String(p.brand).toLowerCase()));
      ...
      await db.markChecked(p.id); out.push({ id: p.id, brand: p.brand, updated: !!g });
    } catch (e) { await db.markChecked(p.id); out.push({ id: p.id, brand: p.brand, error: e.message }); }
  }
  return out;
}
...
async function linkedinTick(account, actor, now) {
  ...
  try {
    ...
    for (const m of msgs) {
      ...
      await db.recordReply(account, actor, p.id, m.text, m.at); if (m.chat_id) await db.setLinkedinIds(p.id, { chat_id: m.chat_id });
      out.replies++;
      E.notifyOwner('Video Outreach reply from ' + p.brand, (p.dm_name || 'Someone') + ' at ' + p.brand + ' replied on LinkedIn:\n\n' + String(m.text || '').slice(0, 1500) + '\n\nNothing has been sent back. Open SitePounce > Video Outreach to answer.');
    }
...
async function tick(account, actor, opts) {
  ...
  try { summary.linkedin = await linkedinTick(account, actor, o.now); } catch (e) { summary.linkedin_error = e.message; }
  try { summary.followups = await autoFollowups(account, actor, base); } catch (e) { summary.followups_error = e.message; }
  return summary;
}

module.exports = { startRun, stepRun, continueRuns, startScheduled, recheck, sendEmail, sendFollowup, autoFollowups, linkedinTick, linkedinSend, pauseLinkedin, tick };
