...
async function refreshProducts(account, actor, p, campaign) {
  ...
  if (!products.length) return { ok: false, error: 'No Shopify or WooCommerce products feed at ' + p.domain + (st.shop && st.shop.status ? ' (Shopify HTTP ' + st.shop.status : '') + (st.shop && st.shop.woo_status ? ', WooCommerce ' + st.shop.woo_status + ')' : (st.shop && st.shop.status ? ')' : '')) + ' and no product links in their ads. Type the product by hand.' };
