  ...
  for (const p of await db.linkedinQueue(account, lim.max_priority, 5)) {
    try {
      ...
      if (s.note_test !== false) {
        ...
        variant = n('soft') < n('offer') ? 'soft' : (n('soft') > n('offer') ? 'offer' : (Math.random() < 0.5 ? 'soft' : 'offer'));
      }
      const noteText = variant === 'soft' ? M.softNote(p) : (p.connection_note || '');
      ...
      await db.setConnectionState(account, actor, p.id, 'Applied', { note: 'Connection request sent automatically with note ' + (variant === 'soft' ? 'B (soft, no offer): ' : 'A (offer): ') + noteText });
      out.requests++;
...
async function dailyReport(account, actor, base, force) {
  ...
  if (r.note_test && r.note_test.length) {
    lines.push('');
    lines.push('CONNECTION NOTE TEST (A offer vs B soft)');
    for (const v of r.note_test) lines.push('- ' + (v.variant === 'soft' ? 'B soft' : 'A offer') + ': ' + v.sent + ' sent, ' + v.accepted + ' accepted (' + (v.sent ? Math.round(100 * v.accepted / v.sent) : 0) + '%), ' + v.replied + ' replied, ' + v.positive + ' positive');
  }
