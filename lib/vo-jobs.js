...
async function refreshProducts(account, actor, p, campaign) {
  ...
  if (!shop.is_shopify) return { ok: false, error: 'No Shopify products feed at ' + p.domain + (shop.status ? ' (HTTP ' + shop.status + ')' : '') };
  ...
  const pick = S.pickProduct(shop.products, (p.ad_analysis && p.ad_analysis.hero_product) || '', featured, { keywords: (c && Array.isArray(c.keywords) ? c.keywords : []).concat(c && c.industry ? [c.industry] : []) });
  await db.setProducts(p.id, shop.products, 'shopify', pick.candidates || []);
