...
async function newLeadAlerts(account, p, base) {
  ...
  return smsAll(targets, (t) => (t.name ? t.name + ', ' : '') + 'new ShekiPro connection: ' + (p.dm_name || 'the contact') + ' at ' + p.brand + ' accepted on LinkedIn. Create an AI video for ' + (M.shortProduct(p.suggested_product_name) || 'their hero product') + ', then paste the link and send here: ' + link, base);
}
...
module.exports = { refreshProducts, replyAlerts, newLeadAlerts, testAlerts, dailyReport, startRun, stepRun, continueRuns, startScheduled, topUp, recheck, sendEmail, sendFollowup, autoFollowups, linkedinTick, linkedinSend, pauseLinkedin, tick };
