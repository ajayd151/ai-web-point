...
const INSIGHT_SYS = [
  ...
  'advice = 2 to 3 specific actions to take today, each with a one line reason why.',
  'objections = objections that actually came up, each with a specific line to handle it.',
  ...
  'Return ONLY JSON: {"found":["..."],"advice":[{"advice":"...","why":"..."}],"objections":[{"objection":"...","handling":"..."}]}.',
].join(' ');
...
async function insightsFor(actor, w) {
...
async function buildDigest(actor, opts) {
  ...
  const rows = ACTIONS
    ...
    .filter((r) => r.n || r.prev);
  const insights = o.insights === false ? null : await insightsFor(actor, w);
