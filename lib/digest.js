...
async function statsFor(actor, from, to) {
  ...
  return {
    ...
    byHour: (rep && rep.byHour) || [],
    byStatus: (rep && rep.byStatus) || [],
  };
}

// Human labels for the statuses, so the email never shows a raw value like 'meeting-booked'.
// Mirrors LEAD_STATUSES in public/app.js. Keep the two in step.
const STATUS_LABELS = {
  '': 'New', contacted: 'Contacted', 'no-answer': "Doesn't answer", interested: 'Interested',
  'appointment-link-sent': 'Appointment link sent', 'meeting-booked': 'Appointment booked',
  callback: 'Call back', 'not-interested': 'Not interested', declined: 'Not interested (via mockup)',
  'invalid-phone': 'Invalid phone', won: 'Won, customer', lost: 'Lost', cleared: 'Cleared',
};
const hhmm = (h) => String(h).padStart(2, '0') + ':00';

// Gentle, factual observations about the shape of the day. The tone is deliberately curious
// rather than accusatory: these go to the person who did the work, and an email that reads like
// a telling-off just gets ignored.
function hourObservations(byHour) {
  const H = {}; (byHour || []).forEach((r) => { H[Number(r.hour)] = r; });
  const hours = []; for (let h = 0; h < 24; h++) hours.push(H[h] || { hour: h, n: 0, uniq: 0, updates: 0 });
  const worked = hours.filter((r) => (r.n || 0) > 0);
  if (worked.length < 2) return [];
  const first = worked[0].hour; const last = worked[worked.length - 1].hour;
  const out = [];

  // the best hour, so it reads as a compliment first
  const peak = worked.slice().sort((a, b) => (b.uniq || 0) - (a.uniq || 0) || (b.n || 0) - (a.n || 0))[0];
  if (peak && (peak.uniq || 0) > 0) {
    out.push('Your strongest hour was ' + hhmm(peak.hour) + ', with ' + peak.uniq + ' business' +
      ((peak.uniq === 1) ? '' : 'es') + ' worked. Handy to know when you are at your sharpest.');
  }

  // quiet stretches INSIDE the working day: either nothing at all, or a big drop from the peak
  const peakU = Math.max.apply(null, worked.map((r) => r.uniq || 0)) || 0;
  const quiet = (r) => (r.n || 0) === 0 || (peakU >= 5 && (r.uniq || 0) <= Math.floor(peakU * 0.2));
  const runs = []; let cur = null;
  for (let h = first; h <= last; h++) {
    const r = H[h] || { hour: h, n: 0, uniq: 0 };
    if (quiet(r)) { if (!cur) cur = { from: h, to: h }; else cur.to = h; }
    else if (cur) { runs.push(cur); cur = null; }
  }
  if (cur) runs.push(cur);
  const notable = runs.filter((r) => (r.to - r.from + 1) >= 2); // a single slow hour is just an hour
  notable.slice(0, 2).forEach((r) => {
    out.push('It went quiet between ' + hhmm(r.from) + ' and ' + String(r.to).padStart(2, '0') + ':59. ' +
      'If that was lunch, a meeting or admin then ignore this, we just spot the pattern, not the reason.');
  });
  return out;
}

// What their day turned into, by status. Leads with the wins where there are any.
function statusObservation(byStatus) {
  const rows = (byStatus || []).filter((r) => (r.uniq || 0) > 0);
  if (!rows.length) return null;
  const total = rows.reduce((a, r) => a + (r.uniq || 0), 0);
  const lab = (s) => STATUS_LABELS[s || ''] || s || 'Cleared';
  const top = rows.slice().sort((a, b) => (b.uniq || 0) - (a.uniq || 0))[0];
  const booked = rows.find((r) => r.status === 'meeting-booked');
  const interested = rows.find((r) => r.status === 'interested');
  const parts = [];
  if (booked) parts.push(booked.uniq + ' appointment' + ((booked.uniq === 1) ? '' : 's') + ' booked');
  if (interested) parts.push(interested.uniq + ' marked interested');
  let line = 'You moved ' + total + ' business' + ((total === 1) ? '' : 'es') + ' along' +
    (parts.length ? (', including ' + parts.join(' and ')) : '') + '. ' +
    'Most went to "' + lab(top.status) + '" (' + top.uniq + ').';
  if (!booked && total >= 8) {
    line += ' Plenty of movement, so the next one to land is probably close.';
  }
  return { line: line, rows: rows.map((r) => ({ label: lab(r.status), n: r.uniq })) };
}
...
async function buildDigest(actor, opts) {
  ...
  return {
    ...
    rows: rows,
    observations: hourObservations(cur.byHour),
    status: statusObservation(cur.byStatus),
    insights: insights,
  };
}
