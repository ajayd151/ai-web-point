...
async function leadTimeline(phone, key, slug) {
  ...
  try {
    await ensure();
    const { rows: items } = await sql`SELECT sent_at, link_sent_at, nudged_at, delivery FROM sms_items
      WHERE (${phone} <> '' AND phone = ${phone}) OR (${key} <> '' AND key = ${key}) ORDER BY sent_at NULLS FIRST LIMIT 8`;
    out.items = items;
    if (phone) {
