...
async function rangeStats(fromIso, toIso) {
  ...
  try {
    await ensure();
    const { rows: a } = await sql`SELECT
        COUNT(*) FILTER (WHERE sent_at IS NOT NULL)::int sent,
        COUNT(*) FILTER (WHERE delivery = 'delivered')::int delivered,
        COUNT(*) FILTER (WHERE reply = 'positive' OR post_reply = 'positive')::int positive,
        COUNT(*) FILTER (WHERE reply = 'negative' OR post_reply = 'negative')::int negative
      FROM sms_items WHERE sent_at >= ${fromIso}::timestamptz AND sent_at < ${toIso}::timestamptz`;
    const { rows: n } = await sql`SELECT COUNT(*)::int c FROM sms_items WHERE nudged_at >= ${fromIso}::timestamptz AND nudged_at < ${toIso}::timestamptz`;
    const { rows: o } = await sql`SELECT source, COUNT(*)::int c FROM sms_optout WHERE at >= ${fromIso}::timestamptz AND at < ${toIso}::timestamptz GROUP BY source`;
    const r = a[0] || {};
    let stop = 0, link = 0; o.forEach((x) => { if (x.source === 'link') link += x.c; else stop += x.c; });
    return { sent: r.sent || 0, delivered: r.delivered || 0, positive: r.positive || 0, negative: r.negative || 0, nudged: (n[0] && n[0].c) || 0, stop: stop, link: link };
...
module.exports = { createCampaign, listCampaigns, campaignItems, setCampaignStatus, dueCampaigns, itemsInState, setItem, sentKeys, addOptout, optoutSet, optoutCounts, stopWindow, sentPhones, dedupeInbound, hourlyBreakdown, byIndustry, rangeStats, ensureBaseVersions, currentMsg, addMsg, setCampaignMessage, messageStats, journey, getItemById, recordInbound, listInbound, setDeliveryBySid, latestItemByPhone, setReply, setPostReply, dueLinkSends, markLinkSent, dueNudges, markNudged, readyToCall, campaignKeys, addItemsToCampaign, stampToppedUp };
