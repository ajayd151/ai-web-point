...
async function createCampaign({ createdBy, name, message, filters, scheduleAt, items, mode, linkMessage, linkDelayMin, nudgeMessage, nudgeHours }) {
  if (!ok()) return null;
  try {
    await ensure();
    const { rows } = await sql`INSERT INTO sms_campaigns (created_by, name, message, filters, schedule_at, mode, link_message, link_delay_min, nudge_message, nudge_hours)
      VALUES (${createdBy}, ${name}, ${message}, ${JSON.stringify(filters || {})}::jsonb, ${scheduleAt}::timestamptz,
              ...
              ${(nudgeMessage || '').trim() || null}, ${Math.min(Math.max(Number(nudgeHours) || 24, 1), 168)})
      RETURNING id`;
    const id = rows[0].id;
...
module.exports = { createCampaign, listCampaigns, campaignItems, setCampaignStatus, dueCampaigns, itemsInState, setItem, sentKeys, addOptout, optoutSet, recordInbound, listInbound, setDeliveryBySid, latestItemByPhone, setReply, setPostReply, dueLinkSends, markLinkSent, dueNudges, markNudged, readyToCall };
