...
async function ensureBaseVersions() {
  ...
  try {
    ...
    for (const c of rows) {
      const ins = await sql`INSERT INTO sms_msg (campaign_id, text, created_by, created_at) VALUES (${c.id}, ${c.message || ''}, ${c.created_by || null}, ${c.created_at}) RETURNING id`;
      const vid = ins.rows[0].id;
      await sql`UPDATE sms_items SET msg_id = ${vid} WHERE campaign_id = ${c.id} AND msg_id IS NULL`;
    }
  } catch (e) { /* next tick */ }
}
async function currentMsgId(campaignId) {
  if (!ok()) return null;
  try { await ensure(); const { rows } = await sql`SELECT id FROM sms_msg WHERE campaign_id = ${campaignId} ORDER BY id DESC LIMIT 1`; return rows[0] ? rows[0].id : null; } catch (e) { return null; }
}
async function addMsg(campaignId, text, by) {
  if (!ok()) return null;
  try { await ensure(); const { rows } = await sql`INSERT INTO sms_msg (campaign_id, text, created_by) VALUES (${campaignId}, ${text}, ${by || null}) RETURNING id`; return rows[0] ? rows[0].id : null; } catch (e) { return null; }
}
...
async function messageStats() {
  ...
  try {
    await ensure();
    const v = await sql`SELECT m.id, m.campaign_id, m.text, m.created_at, c.name AS campaign_name
      FROM sms_msg m LEFT JOIN sms_campaigns c ON c.id = m.campaign_id ORDER BY m.campaign_id, m.id`;
    const base = {};
    v.rows.forEach((r) => { base[r.id] = { id: r.id, campaignId: r.campaign_id, campaignName: r.campaign_name || ('Campaign ' + r.campaign_id), text: r.text, createdAt: r.created_at, sent: 0, delivered: 0, yes: 0, no: 0, stop: 0, optout: 0 }; });
...
module.exports = { createCampaign, listCampaigns, campaignItems, setCampaignStatus, dueCampaigns, itemsInState, setItem, sentKeys, addOptout, optoutSet, optoutCounts, stopWindow, sentPhones, dedupeInbound, hourlyBreakdown, byIndustry, ensureBaseVersions, currentMsgId, addMsg, setCampaignMessage, messageStats, getItemById, recordInbound, listInbound, setDeliveryBySid, latestItemByPhone, setReply, setPostReply, dueLinkSends, markLinkSent, dueNudges, markNudged, readyToCall, campaignKeys, addItemsToCampaign, stampToppedUp };
