...
async function createCampaign({ createdBy, name, message, filters, scheduleAt, items, mode, linkMessage, linkDelayMin, nudgeMessage, nudgeHours, nudges, evergreen }) {
  if (!ok()) return null;
  try {
    await ensure();
    const nlist = cleanNudges(nudges, nudgeMessage, nudgeHours);
    const first = nlist[0] || null;
    const { rows } = await sql`INSERT INTO sms_campaigns (created_by, name, message, filters, schedule_at, mode, link_message, link_delay_min, nudge_message, nudge_hours, nudges, evergreen)
      VALUES (${createdBy}, ${name}, ${message}, ${JSON.stringify(filters || {})}::jsonb, ${scheduleAt}::timestamptz,
              ...
              ${first ? first.message : null}, ${first ? first.hours : 24}, ${JSON.stringify(nlist)}::jsonb, ${!!evergreen})
      RETURNING id`;
    const id = rows[0].id;
...
async function dueNudges(limit) {
  ...
  try {
    await ensure();
    const { rows } = await sql`SELECT i.id, i.name, i.phone, i.category, i.location, i.sent_at, i.nudged_at, i.nudge_count, c.nudges, c.nudge_message, c.nudge_hours, c.created_by
