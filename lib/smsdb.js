...
async function dueNudges(limit) {
  ...
  try {
    await ensure();
    const { rows } = await sql`SELECT i.id, i.name, i.phone, i.category, i.location, i.sent_at, i.nudged_at, i.nudge_count, c.nudges, c.nudge_message, c.nudge_hours, c.created_by, c.from_number
