...
async function markFunnelSite(id, url) {
  if (!ok()) return false;
  try { await ensure(); await sql`UPDATE sms_items SET funnel_site_at = now(), funnel_site_url = ${url || ''} WHERE id = ${id}`; return true; } catch (e) { return false; }
}
...
async function readyToCall(limit) {
  ...
  try {
    await ensure();
    const { rows } = await sql`SELECT i.key, i.name, i.location, i.category, i.phone, i.reply_at, i.post_reply, i.link_sent_at, i.slug, i.view_url, i.funnel_ack_at, i.funnel_site_at, i.funnel_site_url
...
async function leadTimeline(phone, key, slug) {
  const out = { items: [], inbound: [], views: null, optout: null };
  if (!ok()) return out;
  try {
    await ensure();
    const { rows: items } = await sql`SELECT i.sent_at, i.link_sent_at, i.nudged_at, i.delivery, i.view_url, i.slug,
        ...
        i.funnel_ack_at, i.funnel_site_at, i.funnel_site_url,
