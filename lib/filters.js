...
// heuristic only: Google gives no company type, so we read it off the NAME.
// "ltd" = name shows Ltd/Limited/PLC/LLP; "sole" = it doesn't. Verified type needs
// Companies House (a per-business Prowl lookup), not feasible across a whole search.
const LTD_RE = /\b(ltd|limited|plc|llp|l\.t\.d)\b/i;
function matchCompany(name, mode) {
  if (mode === 'ltd') return LTD_RE.test(name || '');
  if (mode === 'sole') return !LTD_RE.test(name || '');
  return true;
}
...
function matchesFilters(b, f) {
  ...
  if (!matchCompany(b.name, f.company || 'any')) return false;
