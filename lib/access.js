...
const PERM_KEYS = ['search', 'seeLeads', 'deleteLeads', 'export', 'mockups', 'sites', 'sms', 'emails', 'callList', 'block'];
...
async function account(req) {
  const email = sessionEmail(req);
  if (!email) return { email: '', access: false, plan: 'none', status: 'anon', reason: 'not_logged_in', member: false, perms: allPerms() };
  if (isComped(email)) return { email, access: true, plan: 'owner', status: 'comped', reason: 'comped', member: false, perms: allPerms() };
  // Team member: free access, riding their account owner's plan (NOT an admin/owner).
  try {
    const tm = await getTeamMember(email);
    if (tm && !tm.suspended) return { email, access: true, plan: 'team', status: 'comped', reason: 'team', member: true, perms: resolvePerms(tm.permissions) };
  ...
  return { email, access, plan, status, reason: access ? 'subscribed' : 'no_subscription', member: false, perms: allPerms() };
}
...
module.exports = { account, requirePaid, isComped, sessionEmail, canDeepDossier, requireDeepDossier, can, requirePermission, PERM_KEYS };
