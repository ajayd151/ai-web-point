...
// Module permissions: unlike the tabs and actions above these default to OFF for a team member and must be ticked.
//   videoOutreach: the whole Video Outreach module (campaigns, prospects, results, Ask AI, help)
//   voReady: Ready to send only (paste the video, send, follow-ups) for whoever makes the videos
//   voSettings: the Settings tab (keys, caps, scoring, alerts, exclusions)
const MODULE_PERM_KEYS = ['videoOutreach', 'voReady', 'voSettings'];
...
function allPerms() { const o = {}; PERM_KEYS.forEach((k) => { o[k] = true; }); MODULE_PERM_KEYS.forEach((k) => { o[k] = true; }); return o; }
// Resolve a stored permission object into every key (tabs and actions true unless explicitly denied, modules only when granted).
function resolvePerms(stored) { const o = {}; const s = stored || {}; PERM_KEYS.forEach((k) => { o[k] = s[k] !== false; }); MODULE_PERM_KEYS.forEach((k) => { o[k] = s[k] === true; }); return o; }
...
// Video Outreach (spec v4 section 7): admin-only for now. The owner always, plus an explicit
// VIDEO_OUTREACH_EMAILS allow-list. Apex plan gating arrives in Phase 4.
function videoOutreachList() {
  return (process.env.VIDEO_OUTREACH_EMAILS || '').split(',').map((s) => s.trim().toLowerCase()).filter(Boolean);
}
// Phase 4 plan gating: set VIDEO_OUTREACH_PLANS (e.g. "apex") and subscribers on those plans with an
// active subscription get the module too. Unset = allow-list only (the default while it is internal).
function videoOutreachPlans() { return (process.env.VIDEO_OUTREACH_PLANS || '').split(',').map((s) => s.trim().toLowerCase()).filter(Boolean); }
function canVideoOutreach(email, acct) {
  const e = String(email || '').toLowerCase();
  if (!e) return false;
  if (e === ownerEmail()) return true;
  const opUser = String(process.env.APP_USERNAME || '').toLowerCase();
  if (opUser && e === opUser) return true;
  if (!opUser && e === 'user') return true;
  if (isComped(e)) return true;
  if (videoOutreachList().includes(e)) return true;
  if (acct && acct.member && acct.perms && (acct.perms.videoOutreach || acct.perms.voReady || acct.perms.voSettings)) return true; // granted by the workspace owner in Admin, Team
  const plans = videoOutreachPlans();
  if (plans.length && acct && acct.access && ACTIVE_STATUSES.includes(String(acct.status || '')) && plans.includes(String(acct.plan || '').toLowerCase())) return true;
  return false;
}

// What a Video Outreach user may reach: owners and subscribers get everything, a team member only what was ticked.
function voLevel(acct) {
  if (!acct || !canVideoOutreach(acct.email, acct)) return { all: false, ready: false, settings: false };
  if (!acct.member) return { all: true, ready: true, settings: true };
  const p = acct.perms || {};
  return { all: !!p.videoOutreach, ready: !!(p.videoOutreach || p.voReady), settings: !!p.voSettings };
}
...
async function account(req) {
  ...
  try {
    ...
    if (tm && !tm.suspended) return { email, access: true, plan: 'team', status: 'comped', reason: 'team', member: true, perms: resolvePerms(tm.permissions), limits: cleanLimits(tm.limits), mustChange: !!tm.must_change };
...
module.exports = { account, requirePaid, isComped, sessionEmail, canDeepDossier, requireDeepDossier, canVideoOutreach, voLevel, videoOutreachPlans, can, requirePermission, PERM_KEYS, ACTION_PERM_KEYS, VIEW_PERM_KEYS, MODULE_PERM_KEYS, LIMIT_KEYS, cleanLimits };
