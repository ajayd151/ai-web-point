...
function canVideoOutreach(email) {
  const e = String(email || '').toLowerCase();
  if (!e) return false;
  if (e === ownerEmail()) return true;
  const opUser = String(process.env.APP_USERNAME || '').toLowerCase();
  ...
  return videoOutreachList().includes(e);
}
...
module.exports = { account, requirePaid, isComped, sessionEmail, canDeepDossier, requireDeepDossier, canVideoOutreach, can, requirePermission, PERM_KEYS, ACTION_PERM_KEYS, VIEW_PERM_KEYS, LIMIT_KEYS, cleanLimits };
