  ...
  try {
    ...
    return {
      ...
      weaknesses: Array.isArray(p.weaknesses) ? p.weaknesses.map((w) => ({
        ...
        solution: String((w && w.solution) || '').trim(),
...
async function gatherDossier({ slug, name, location, category, phone, mini }) {
  ...
  // Mini skips the OpenAI synthesis (the real cost): just Google reviews + comments + the
  // competitor table + Companies House. The AI call briefing (opener, weaknesses, ammunition,
  // objections) is the "full" upgrade.
  const synthesis = mini
    ? { services: [], reputationSummary: '', strengths: [], weaknesses: [], ammunition: [], objections: [], openingLine: '' }
    : await synthesize({ name, location, category: cat, ch, google, competitors, reviews });
  ...
  const dossier = {
    ...
    google,
    competitors,
    reviews: reviews,           // the actual recent review comments, shown in mini and full
    reviewCount: reviews.length,
    mini: !!mini,               // marks a Google-only dossier so "full" knows to regenerate
...
module.exports = { gatherDossier, readDossier, companiesHouse };
