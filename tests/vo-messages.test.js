...
for (const { input } of rows) {
  test('messages pass the C5 post-checks: ' + input.brand, () => {
    ...
    for (const k of ['message_a', 'message_b']) {
      ...
      assert.ok(paras.every((p) => p.trim() && !p.includes('\n')), k + ' paragraphs must be separated by a blank line');
      assert.ok(out[k].includes('Thanks,') && out[k].endsWith(M.DEFAULT_PROFILE.sender_first + ', ' + M.DEFAULT_PROFILE.sender_title), k + ' must end with the sign-off');
    }
