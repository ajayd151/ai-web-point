...
- **Login = Clerk** (Google + email). Front-end loads ClerkJS (gated by `window.AIWP_CLERK.enabled`); the Clerk session is exchanged once for the existing `aiwp` cookie via `api/clerk-session.js` (so the ~16 protected endpoints are unchanged). Access is allow-listed by email (`ALLOWED_EMAILS`) until public sign-ups open. See `lib/clerkauth.js` (zero-dep JWKS verify).
...
- **Still pending:** Stripe **live keys** + recreate webhook/portal in live mode (use the `www` URL); flip `SIGNUP_OPEN=1`; in-app **Super Admin** (see all customers + usage); per-tier **feature gating + credits**; UX polish (full-page sign-up instead of the Clerk popup, more top-nav items, mandatory first/last name).
...
| Auth (Clerk) | `CLERK_SECRET_KEY?`, `ALLOWED_EMAILS?`, `CLERK_ISSUER?` | Clerk session auth (Phase 1 B). `CLERK_SECRET_KEY` present = the backend on-switch (`api/clerk-session.js`); `ALLOWED_EMAILS` (comma list) = who may sign in (empty = nobody, safe default); `CLERK_ISSUER` defaults to the dev instance `https://major-cod-60.clerk.accounts.dev`. The publishable key is public and lives in `public/index.html` (`window.AIWP_CLERK`), gated by `enabled`. Verified with zero-dep JWKS (`lib/clerkauth.js`) then exchanged for the `aiwp` cookie, so other endpoints are unchanged. |
