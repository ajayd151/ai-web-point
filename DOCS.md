...
### Multi-user SaaS (Phase 1, 2026-06-30)
Turning the operator tool into a sellable subscription. **All of this is owner-safe: you (the owner) keep the exact same app and data; isolation/gating only affects future paying customers.**
- **Login = Clerk** (**production instance, live**, email + password; Google was disabled to skip the Google OAuth app). Front-end loads ClerkJS from `clerk.sitepounce.com` (gated by `window.AIWP_CLERK.enabled`, `pk` = `pk_live_...`); the Clerk session is exchanged once for the existing `aiwp` cookie via `api/clerk-session.js` (so the ~16 protected endpoints are unchanged). Backend verifies the session JWT against `CLERK_ISSUER=https://clerk.sitepounce.com` via zero-dep JWKS (`lib/clerkauth.js`). Prod Clerk needed 5 CNAME DNS records in Cloudflare (clerk/accounts/clkmail + 2 DKIM, all DNS-only) which removed the "Development mode" badge. **Note:** Clerk's own verification emails send from the (brand-new) `clerk.sitepounce.com` domain, so they can land in spam on strict providers (Outlook) until the domain warms up, DNS/DKIM is correct, it is reputation.
- **Billing = Stripe** (test/sandbox first). 3 products (Scout £29 / Hunter £59 / Apex £129 / month, GBP). `api/stripe-checkout.js` (pick a plan -> hosted Checkout), `api/stripe-confirm.js` (post-redirect, records the plan in the Neon `users` table), `api/stripe-portal.js` (self-serve manage/cancel), `api/stripe-webhook.js` (renewals/cancels; verified by re-fetching from Stripe). `lib/stripe.js` = zero-dep Stripe REST. **Customer Portal + webhook are LIVE in test mode** (a "Manage billing" button in the top-right opens the portal for paying subscribers). **GOTCHA:** the webhook URL must be `https://www.sitepounce.com/...` (the `www` is canonical; the apex 308-redirects and Stripe does not follow redirects, so the apex URL silently fails every delivery). The webhook needs no signing secret (our handler re-fetches the object from Stripe to verify). **TODO for live:** recreate the webhook + portal config in Stripe live mode and paste live keys.
- **New-subscriber emails** (`lib/email.js`): on subscription, a **welcome** email to the customer (login link, no password since Clerk handles sign-in) + a **"new customer"** notice to you, via SendGrid, from `hello@sitepounce.com` (domain-authenticated). Deduped via a `welcomed` flag. IMPORTANT pattern: the send is **awaited** before the HTTP response (Vercel freezes the function after responding, so fire-and-forget never sends).
- **Paywall gating** (`lib/access.js`): the Clerk cookie proves *who* you are; paid access is decided separately. The costly endpoints (`search`, `generate`, `prowl`, `pounce`, `grammar`) call `requirePaid` and return **402** unless the user has an active/trialing subscription. **Comped (free, no card): the owner, the legacy operator login, and any `ALLOWED_EMAILS`.** The front-end reads `api/me.js` and shows the app (subscribers) or a **paywall** plan-picker (signed in, no plan yet). Fails closed.
- **Open-signups switch** (`SIGNUP_OPEN`): `clerk-session.js` issues a cookie to allow-listed emails only by default (live app stays owner-only, unchanged). Set `SIGNUP_OPEN=1` to let anyone sign in; paid features stay gated by subscription. This is the "go public" flip, done alongside switching Stripe to live keys.
- **Legal** (`public/terms.html`, `public/privacy.html`): launch drafts (England & Wales, named sub-processors, per-tier retention, cookies, ICO). Linked from the landing footer + a consent line on the pricing section and the paywall. Flagged for solicitor review before launch.
- **Multi-tenant data separation** (`lib/tenant.js`): every per-customer artifact is namespaced by the signed-in user. **Owner = no prefix (unchanged); other customers get `u/<hash>/` blob paths and a `<16hex>--` slug marker.** Mockup/site/dossier/note **slugs** carry the tenant (kept globally unique so the public `/v/` `/s/` links need no change); the list/dashboard/hot-leads views and the per-tenant **usage quotas** + **call list** filter by `ownsSlug`/`tenantPrefix`. So a customer only ever sees their own leads, mockups, sites and stats.
- **LIVE as of 2026-07-01:** Stripe live keys + live products/prices/webhook/portal, `SIGNUP_OPEN=1`, and **Clerk production** are all done. Proven with real end-to-end payments. A branded **welcome page** (`#welcome`) shows after subscribing (replaced the alert popup).
- **Still pending:** in-app **Super Admin** (see all customers + usage); per-tier **feature gating + credits**; **full-page sign-up** (Clerk popup is dismissable, clicking the backdrop loses input); **Upgrade / change-plan** in the in-app Manage; **redesign the paywall** into proper pricing cards; **remove "Secured by Clerk"** (= Clerk Pro $25/mo); more top-nav items; mandatory first/last name.
...
| Auth | `APP_USERNAME`, `APP_PASSWORD` | Legacy login; cookie HMAC keyed by the password. Stays live + as a fallback even after Clerk is on. |
| Billing (Stripe) | `STRIPE_SECRET_KEY?`, `STRIPE_PRICE_SCOUT/HUNTER/APEX?`, `APP_BASE_URL?` | Subscriptions (Phase 1 C). Secret key present = checkout/portal/webhook turn on; the three price IDs map a plan to its Stripe price; `APP_BASE_URL` (default the request host) builds the success/cancel/return URLs. |
| Subscriber emails | `SITEPOUNCE_FROM_EMAIL?` | The "from" for welcome/admin emails (`hello@sitepounce.com`, domain-authenticated in SendGrid). If unset, falls back to the verified `ERROR_EMAIL_FROM`, still branded "Site Pounce" + reply-to hello@. Admin recipient = `LEAD_EMAIL_TO`/`ERROR_EMAIL_TO`. |
| Multi-tenant | `OWNER_EMAIL?` | The owner whose data stays at the legacy root namespace (defaults to `ALLOWED_EMAILS[0]` / ajay@aimpro.co.uk). Every other signed-in customer is isolated under `u/<hash>/` + a `<16hex>--` slug. |
| Sign-ups | `SIGNUP_OPEN?` | Unset/`0` = allow-list only can sign in (default, owner-only live app). `1` = anyone can sign in via Clerk; paid features still gated by an active subscription (`lib/access.js`). Flip to `1` when going public with live Stripe. |
| Auth (Clerk) | `CLERK_SECRET_KEY?`, `ALLOWED_EMAILS?`, `CLERK_ISSUER?` | Clerk session auth (Phase 1 B). `CLERK_SECRET_KEY` present = the backend on-switch (`api/clerk-session.js`); `ALLOWED_EMAILS` (comma list) = who may sign in (empty = nobody, safe default); `CLERK_ISSUER` is **now set to the production instance `https://clerk.sitepounce.com`** (defaults to the old dev instance if unset). The publishable key is public and lives in `public/index.html` (`window.AIWP_CLERK`, now `pk_live_...` + domain `clerk.sitepounce.com`), gated by `enabled`. Verified with zero-dep JWKS (`lib/clerkauth.js`) then exchanged for the `aiwp` cookie, so other endpoints are unchanged. |
...
| Per-user limits | `USER_LIMITS?` | JSON keyed by lowercase email, e.g. `{"mate@x.com":{"search":10,"generate":5,"prowl":5,"pounce":2}}`. Overrides the global `LIMIT_*` for that person only (kinds you omit fall back to the default). Lets you cap a free/comped teammate below your own limits (`lib/ratelimit.js` `limitFor(kind,email)`). |
