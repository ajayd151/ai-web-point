...
- **Still pending:** Stripe **live keys** + Customer Portal + webhook (dashboard steps); flip `SIGNUP_OPEN=1`; in-app **Super Admin** (see all customers + usage); per-tier **feature gating + credits**; UX polish (full-page sign-up instead of the Clerk popup, more top-nav items, mandatory first/last name).
