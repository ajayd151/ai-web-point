...
async function voOpenReady() {
  ...
  el.querySelectorAll('.vo-card').forEach((card) => {
    ...
    const wording = () => { const u = urlBox.value.trim(); let t = textBox.value; if (mode() === 'attachment') t = t.replace(/:\s*(\[insert URL here\]|https:\/\/\S+)/, ', it is attached below'); else { t = t.replace(/, it is attached below/, ': ' + (u || '[insert URL here]')); if (t.includes('[insert URL here]') && u) t = t.split('[insert URL here]').join(u); else if (lastUrl && u !== lastUrl && t.includes(lastUrl)) t = t.split(lastUrl).join(u || '[insert URL here]'); } textBox.value = t; lastUrl = u; };
