...
function voRenderCampaigns() {
  ...
  const rows = VO.campaigns.map((c) => '<tr data-id="' + c.id + '">' +
    ...
    '<td>' + voSchedText(c) + '</td>' +
    '<td><span class="vo-status st-' + esc(c.status) + '">' + esc(c.status) + '</span>' + (c.last_run_at ? '<div class="muted vo-small">last run ' + esc(voStamp(c.last_run_at)) + '</div>' : '') + (c.automation && c.automation.auto_connect ? '<div class="vo-small vo-yes">auto requests on' + (c.automation.auto_topup === false ? '' : ', auto top-up') + '</div>' : '') + '</td>' +
