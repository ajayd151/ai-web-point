...
async function voOpenReady() {
  ...
  const cards = (d.prospects || []).map((p) => '<div class="vo-card' + (p.source === 'demo' ? ' vo-demo' : '') + '" data-id="' + p.id + '">' + (p.source === 'demo' ? '<div class="vo-banner note" style="margin:0 0 8px">📘 <b>Example only, not a real brand.</b> Demo Brand and Jamie Example are made up, so you can see the layout before real acceptances arrive. Real cards look the same without this box. <button class="primary sm" id="vo-demo-remove">Remove the example</button></div>' : '') + '<div class="vo-bar" style="margin:0 0 6px"><div><b>' + esc(p.brand) + '</b> ' + voPrio(p) + ' <span class="muted vo-small">' + esc(p.dm_name || '') + (p.dm_title ? ', ' + esc(p.dm_title) : '') + ' · ' + esc(p.campaign_name || '') + ' · connected ' + esc(voStamp(p.linkedin_connected_at)) + '</span></div><div>' + voLink(p.dm_linkedin, 'LinkedIn') + ' <button class="ghost sm vo-open">Open</button></div></div>' +
    voShortlist(p) +
    '<div class="vo-small">Video URL: <input type="url" class="vo-ready-url" value="' + esc(p.video_url || '') + '" placeholder="https://…" style="width:60%;max-width:420px" /> <span class="muted">paste it here once, it drops into the message where it says [insert URL here]</span></div>' +
  ...
  el.querySelectorAll('.vo-card').forEach((card) => {
    ...
    const urlBox = card.querySelector('.vo-ready-url'), textBox = card.querySelector('.vo-ready-text'); let lastUrl = (urlBox.value || '').trim();
    urlBox.addEventListener('input', () => { const u = urlBox.value.trim(); const cur = textBox.value; if (cur.includes('[insert URL here]')) textBox.value = cur.split('[insert URL here]').join(u || '[insert URL here]'); else if (lastUrl && cur.includes(lastUrl)) textBox.value = cur.split(lastUrl).join(u || '[insert URL here]'); lastUrl = u; });
