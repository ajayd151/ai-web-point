...
async function voEditCampaign(id) {
  ...
  const html = '<div class="vo-bar"><h3 style="margin:0">' + (c ? 'Edit campaign' : 'New campaign') + '</h3><div><button class="ghost" id="vo-back">← Campaigns</button> ' + (c ? '<button class="ghost" id="vo-goprospects">Prospects →</button> <button class="primary" id="vo-run">▶ Run now</button> ' : '') + '<button class="primary" id="vo-save">Save</button></div></div>' +
    ...
    (c ? '<div class="vo-bar" style="margin-top:16px"><h3 style="margin:0">Import prospects</h3><div><button class="ghost" id="vo-import-fix">📥 Import v12 tracker</button> <label class="ghost vo-file">📄 Import CSV <input type="file" id="vo-import-file" accept=".csv,text/csv" hidden /></label></div></div>' +
      '<h3>Runs</h3>' + (runs.length ? '<div class="tgt-scroll"><table class="cust-table vo-table"><thead><tr><th>Started</th><th>Kind</th><th>Status</th><th>Raw</th><th>Qualified</th><th>Parked</th><th>Disq.</th><th>Cost</th><th>Stopped because / errors</th></tr></thead><tbody>' +
        runs.map((r) => { const k = r.counts || {}; return '<tr><td>' + esc(voStamp(r.started_at)) + '</td><td>' + esc(r.kind) + '</td><td>' + esc(r.status) + '</td><td class="num">' + (k.raw != null ? k.raw : (k.total || '')) + '</td><td class="num">' + (k.qualified != null ? k.qualified : (k.imported || '')) + '</td><td class="num">' + (k.parked || '') + '</td><td class="num">' + (k.disqualified || '') + '</td><td>' + voMoney(r.actual_cost) + '</td><td class="vo-small">' + esc((r.state && r.state.stop) || '') + (r.errors && r.errors.length ? '<div class="muted">' + esc(r.errors.slice(0, 3).join('; ')) + '</div>' : '') + '</td></tr>'; }).join('') + '</tbody></table></div>' : '<p class="muted">No runs yet.</p>') : '');
