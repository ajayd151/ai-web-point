...
async function voOpenSettings() {
  ...
  el.innerHTML = '<h3>Settings</h3><p class="muted view-sub">Secrets live only in Vercel environment variables; this screen shows whether each one is set, never the value. Everything else is edited here.</p>' +
    ...
    '<div class="vo-card"><h4 style="margin:0 0 8px">Reply alerts by SMS</h4><div class="vo-help">Mobiles to text when a reply comes in, one per line with the country code (for example +447866555555). The text reads "Positive reply for ShekiPro from Dan at Create via LinkedIn" with what they want and what to do next. Uses the SitePounce SMS number.</div><textarea id="vo-alert-mobiles" rows="3">' + esc(((d.alerts || {}).mobiles || []).join('\n')) + '</textarea><div style="margin-top:6px"><label class="vo-chk"><input type="checkbox" id="vo-alert-all"' + ((d.alerts || {}).all_replies ? ' checked' : '') + '/> text me for every reply, not only Positive and Question</label></div><div style="margin-top:6px"><button class="primary sm" id="vo-alert-save">Save alerts</button></div></div>' +
