...
var VO_HELP = {
  ...
  'Sender title': 'Your role, written after the name in the sign-off, for example "Co-founder at Shekipro".',
...
async function voEditCampaign(id) {
  ...
  const html = '<div class="vo-bar"><h3 style="margin:0">' + (c ? 'Edit campaign' : 'New campaign') + '</h3><div><button class="ghost" id="vo-back">← Campaigns</button> ' + (c ? '<button class="ghost" id="vo-goprospects">Prospects →</button> <button class="primary" id="vo-run">▶ Run now</button> ' : '') + '<button class="primary" id="vo-save">Save</button></div></div>' +
    ...
    row('Sender first name', '<input type="text" data-sp="sender_first" value="' + esc(sp.sender_first || '') + '" />') + row('Sender title', '<input type="text" data-sp="sender_title" value="' + esc(sp.sender_title || '') + '" placeholder="Co-founder at Shekipro" />', 'appears after the name in the sign-off: "Aj, Co-founder at Shekipro"') + row('Sign-off', '<input type="text" data-sp="signoff" value="' + esc(sp.signoff || '') + '" />') +
    ...
    row('Placeholders', '<span class="vo-small">{first} {service_name} {service_desc} {observation} {product} {video_url} {signoff} {sender_first} {sender_title} {sender_line} {offer_line} {pilot_line} {seasonal_event}. A blank line separates paragraphs. Blank box = the default template.</span>') +
