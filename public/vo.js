...
var VO_HELP = {
  ...
  'Connection note test': 'Every automatic request alternates two notes. A is the prospect\'s connection note: says what we do and offers the free sample. B is a soft note that only mentions their product ads on Meta and asks to connect, giving nothing away. The Results tab and the daily report show acceptance and replies per note. Judge after 60 to 80 of each.',
...
async function voOpenResults(campaignId) {
  ...
  el.innerHTML = repHtml + '<div class="vo-bar"><div><h3 style="margin:0">Results</h3><p class="muted view-sub" style="margin:2px 0 0">Reply, call and pilot rates by priority band and by message variant, so the weights and the A/B choice can be tuned from data. Rates are against contacted prospects.</p></div>' +
    ...
    (rep && rep.note_test ? '<h4>Connection note test' + voHelp('Connection note test results') + '</h4><div class="vo-fit"><table class="cust-table vo-table"><thead><tr><th>Note</th><th class="num">Sent</th><th class="num">Accepted</th><th class="num">Accept rate</th><th class="num">Open</th><th class="num">Replied</th><th class="num">Positive</th></tr></thead><tbody>' +
      (['offer', 'soft'].map((k) => { const v = rep.note_test.find((x) => x.variant === k) || { sent: 0, accepted: 0, open: 0, replied: 0, positive: 0 }; return '<tr><td><b>' + (k === 'soft' ? 'B soft' : 'A offer') + '</b><div class="muted vo-small">' + (k === 'soft' ? 'mentions their product ads on Meta, asks to connect, no offer' : 'says what we do and offers the free sample video') + '</div></td><td class="num">' + v.sent + '</td><td class="num">' + v.accepted + '</td><td class="num">' + (v.sent ? Math.round(100 * v.accepted / v.sent) + '%' : '') + '</td><td class="num">' + v.open + '</td><td class="num">' + v.replied + '</td><td class="num">' + v.positive + '</td></tr>'; }).join('')) + '</tbody></table></div>' : '') +
...
async function voOpenSettings() {
  ...
  el.innerHTML = '<h3>Settings</h3><p class="muted view-sub">Secrets live only in Vercel environment variables; this screen shows whether each one is set, never the value. Everything else is edited here.</p>' +
    ...
    '<div class="vo-card"><h4 style="margin:0 0 8px">LinkedIn caps <span class="muted vo-small">(hard cap ' + (d.limits ? d.limits.hard_daily_requests : 25) + ' requests a day)</span></h4><table class="vo-kv"><tbody>' +
      ...
      kv('Connection note test', '<select data-li="note_test"><option value="on"' + (li.note_test === false ? '' : ' selected') + '>On, alternate A and B 50:50</option><option value="off"' + (li.note_test === false ? ' selected' : '') + '>Off, always the offer note (A)</option></select>' + voHelp('Connection note test')) +
